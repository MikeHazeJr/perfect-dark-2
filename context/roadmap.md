...
| E16 | **Wire Format Versioning** | SHIPPED-V58 / REGRESSION-GATED | `NET_PROTOCOL_VER` is v58. The wire adds an epoch-bound authority-and-peer post-load stage lifecycle plus an atomic canonical NPC checkpoint to typed manifest distribution, exact authenticated player identity, transactional lobby/stage launch, cutscene authority, and endpoint-scoped reconnect state. Mixed builds are refused. Frozen complete automation, the native-source guard, and all seven ordinary-client protocol-v58 paths pass on exact client `5DA75BE6...`; broader release revalidation remains under T-ENGINE-004/V-010. |
...
5. **AllInOne cull Phase 2 (E17)** -- finish the 15-step sequencing per Section H decisions. v44 protocol bump live (current wire is v46 after subsequent bumps).
...
- AllInOne Phase 2 merged + protocol v46 stable (was v44 at original authoring; current is v46).
...
- ENet protocol at v46 (v44 at the time of original authoring; v45 added spawn-weapon mode S483, v46 added mandatory mod-transfer SHA-256 digest + cutscene network semantics S507/S511)
...
- **Network-protocol bump cadence.** v46 today (was v44 at original authoring; v45 spawn-weapon, v46 mandatory mod-transfer digest + cutscene). Expected future bumps: P4-B if approved, full string-only wire surface (E5 Phase 3), spectator/theater finalization, voice integration, federation handshake. Mixed-version tolerance via the existing reject-at-handshake pattern; cumulative bumps risk scaring tester populations into stale builds.
...
- `context/pillars/*.md` (live state per architectural pillar)
- `context/constraints.md` (active and removed constraints)
- `context/session-log.md` (recent work, rolling window)
- `context/tasks.md` (active punch list)
- `context/working-preferences.md` (collaboration shape)
- All design docs in `context/designs/` (sub-bucketed by pillar)
- Audits in `context/audits/` (14-day window per `retention.md`)
- Historical archive at `_old/` (prior context tree, including `_old/ADR-001`, `_old/ADR-002`, `_old/ADR-003`, `_old/ADR-004`; ADR-003 catalog content folded into `pillars/catalog.md`)

Originally authored 2026-04-27. Last refreshed 2026-04-30 (rebuild port; protocol references updated v44 -> v46; "Current critical path" section added; sources list reflects new structure).
