...
**Rendering note (2026-05-19)**: B-346 credits/fog solid-square artifact is fixed pending playtest. The earlier c3746 note was false closure: it only masked one `A_FOG` field. The current patch decodes fast3d blender fields for c1/c2 fog and translucent memory modes, adds a fog-alpha shader option, and pins the behavior with static coverage. Isolated `gfxalpha` client/updater build, tests build, and focused `[rendering][fast3d][fog][static][b346]` run passed; Mike is manually testing credits and fog/additive stage effects from `.claude/session-builds/gfxalpha/PerfectDark.exe`. Kanban card `c134` tracks the fix as active/pending completion with the manual credits/fog playtest subtask.

**Menus note (2026-05-19)**: Settings > Debug now has an offline-only "Go to Credits" scene shortcut for faster credits rendering playtests. Static coverage pins the button, `GRID_STAGE_CREDITS` handoff, and netplay disable guard. Build/test verification is pending in session `dbgcred`.
