...
2. **Custom body/head full runtime equivalence**
   ...
   - 2026-06-09: FULLY DESIGNED and ready for a one-pass implementation. See `context/designs/modding/body-head-private-slot-allocator.md`. Verified-contained blast radius: Option B (separate `_TOTAL` for arrays/bounds, base 152 for population loops -- growing the raw count would pollute `s_pickRandomByGender`). Reverse-map confirmed feasible (`catalogIdByRuntime` scans by `runtime_index`, `RT_CACHE_SIZE`=1024). New `assetcatalog_body_head_slots.c` mirrors `assetcatalog_weapon_slots.c`. ~12 coordinated files on the character render path; live custom-body render is the only B-801-blocked step. Held from this session because it is the most render-sensitive change and the context was saturated; deferred to a focused full-context pass per the data-layout-change discipline.
