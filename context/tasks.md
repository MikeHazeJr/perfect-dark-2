...
- **s036-08 Menu graph completion** -- BACKLOG, multi-session lane. The raw 174 port + 160 src grep count overcounts via comment mentions (e.g. pdgui_menu_warning.cpp's hit was a comment, not a call). True target inventory requires per-file audit. L.16-L.39 in design doc shows 24 prior single-edge commits; realistic per-session throughput is 2-4 edges. Track as long-running.
