...
- **Step 5** (retirement): delete `base/*.pdbase` + extractor scripts + parity checks. Drop legacy loose-files writers in `pdgui_theme.cpp` (`s_writePng`, `s_writeNinesliceJson`, CRC32 helpers) now unreferenced after the .pdui migration. Add grep-guard test pinning. Roll PAL/JPN locale extension into Step 5 cleanup if not folded earlier.
