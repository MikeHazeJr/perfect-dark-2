...
**Step 4 status (2026-05-03, worktree `affectionate-hawking-f01503`)**: SHIPPED. Universal directory walker (`loaderWalkerLoadAll`) walks `data/<romid>/<class>/*.pd<ext>` for all 13 universality kinds and registers a catalog row per file via the existing `assetCatalogRegister*` API. Per-kind walker functions sharing a `loader_walker_common.{h,c}` scaffold (iterate dir + open ZIP via `modArchiveOpen` + parse manifest envelope + dispatch to per-kind callback). Auto-detects ZIP vs plain JSON by 2-byte file magic so the dual-shape `.pdanim` (weapon vs character) flows through one callback. Non-destructive overlay: scaffold short-circuits via `assetCatalogResolve(id)` so existing in-binary rows from `assetCatalogRegisterBaseGame` are counted without overwrite (preserves bootstrap fields like `model_file`); new rows (~1208 chr animations + disk-only IDs) get fresh registrations. New files: `port/include/loader_walker.h`, `port/include/loader_walker_common.h`, `port/src/loader_walker.c`, `port/src/loader_walker_common.c`, plus 13 `port/src/loader_walker_<kind>.c` sources. Boot wiring: `port/src/main.c` Step 4 block immediately after the Step 3b part 2 `.pdui` block and before `catalogBuildRuntimeCaches`. The `.pdbase` parser block earlier in the boot path stays primary for pool population (parity-bounded fallback for Step 5 retirement).

**Remaining**:

- **Step 5** (retirement): delete `base/*.pdbase` + extractor scripts + parity checks. Migrate pool population from `loaderPdbaseScan` onto the walker (parse weapon functions / ammos / aim/noise/recoil / partvis / etc. from the per-asset `.pdwpn` envelope). Drop legacy loose-files writers in `pdgui_theme.cpp` (`s_writePng`, `s_writeNinesliceJson`, CRC32 helpers) now unreferenced after the .pdui migration. Add grep-guard test pinning. Roll PAL/JPN locale extension into Step 5 cleanup if not folded earlier.
