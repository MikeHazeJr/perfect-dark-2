...
- **B-341 load-screen crash before intro** -- FIXED-PENDING-PLAYTEST 2026-05-18. `Build\pd-client.log` showed an AV immediately after `LOAD: bgBuildTables done` on CI/title load. Root cause was the B-339 world-mesh builder calling `bgFindRoomVtxBatches()` before demand-loaded room gfxdata existed. `meshWorldAddRenderedRoom()` now loads the room first and falls back to room geo if gfxdata is still unavailable. Verified isolated `loadcr` client/updater, tests, server, and focused `[physics][jump]` PASS. Needs Mike playtest: normal launch should reach intro/main menu without the `ACCESS_VIOLATION`.
