...
**Modding note (2026-05-20)**: External-format `.pdmod` card `c3809` is active. Completed slices: format contract, shared INI/archive scanner, metadata-family templates, audio/music standard-file playback, UI/font/lang standard-file usage, `c3809-s6` models/maps/animations, and `c3809-s7` packer/exporter integration. Authored root-`mod.json` `.pdmod` archives reject `.bin` payloads; `.bin` remains legacy-only outside the external authoring contract. Models/maps/animations are build-verified through readable cache outputs and engine-facing modeldef/colmesh/animation payloads. Modding Hub folder-to-`.pdmod` creation now validates canonical external-layout folders before writing archives, rejects authored `.bin` packer inputs, generates missing commented INI templates, validates referenced source files/unsafe paths, and reports detailed errors through `modpackPdmodLastError()`. Current slice: `c3809-s8` final compatibility + validation. Static/build coverage now pins the legacy `.pd*` walker matrix and Public Mods folder sharing packages through validated `.pdmod`; in-game load/playback checks remain pending.
...
- **ACTIVE 2026-05-20: External-format `.pdmod` pipeline (Kanban c3809).** Mike selected full external formats and clarified that authored archives should not need `.bin` files at all. Current contract: root `mod.json` remains, asset content becomes standard editable files plus grouped `.ini` / `.tsv` metadata, `.bin` payloads are invalid in authored `.pdmod` archives, and engine-native binary data may exist only as private readable runtime cache generated from source files. Design: [designs/modding/external-format-pdmod-pipeline.md](designs/modding/external-format-pdmod-pipeline.md). Completed slices: tracking/format contract, shared INI + archive scanner foundation, grouped metadata-family authoring/templates, audio/music externalization/VFS playback, UI/font/lang externalization, models/maps/animations, and packer/exporter integration. `c3809-s8` is active with static/build coverage complete for the legacy `.pd*` walker matrix and Public Mods folder-to-`.pdmod` sharing; remaining closure gate is in-game load/playback validation for legacy `.pd*`, external-layout `.pdmod`, loose folder mods, and Public Mods transfer/install behavior.
