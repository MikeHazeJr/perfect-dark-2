...
**Status**: c036 active 2026-05-12 sprint sprc036. 7 of 8 subtasks DONE; only s036-08 (menu graph completion, multi-session lane) remains backlog. Per [designs/input/input-universality-and-transitions.md](designs/input/input-universality-and-transitions.md), Cohorts 1-4 shipped earlier (layer types + scene events, layer push/pop, IMC ownership migration, per-player cutscene state); this sprint closed the Cohort 5-8 controller surface except the menu graph chip-work.

**Subtask state (2026-05-12, end of sprc036 continuation)**:
...
- **s036-08 Menu graph completion** -- BACKLOG, multi-session lane. The raw 174 port + 160 src grep count overcounts via comment mentions (e.g. pdgui_menu_warning.cpp's hit was a comment, not a call). True target inventory: ~35 real call sites across 12 files (LF-1 in 2026-05-13 audit). 2026-05-13 slice (this session) migrated 5 raw `menuPopDialog()` sites across 4 small priority screens (Agent Create save+cancel, Challenges back, MP Team Setup done, MP Player Config close) via new EDGE_POP graph nodes + 4 static pd-test pins (`[input][menu_graph][agent/challenges/teamsetup/playerconfig][static]`). See L.60 in `designs/input/input-universality-and-transitions.md`. Remaining surface ~30 sites across 8 larger files (training.cpp 11 pops, solomission.cpp leftover pops, cheats.cpp pops, mpadvanced/mpsetup/mpsettings/botsetup/controldiagram/mainmenu push residue). Continues as long-running lane at ~5 edges/session.
