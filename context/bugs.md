...
| **B-129** | HIGH | AV on mission completion — `endscreenPrepare()` called `filemgrSaveOrLoad()` which on PC hits `pakFindBySerial()==-1` (no Controller Pak) → pushes unregistered `g_PakNotOriginalMenuDialog` (DANGER + fn-ptr title) → type-based `renderDangerDialog` → `getDialogTitle` → `langSafe((s32)fn_ptr)` → OOB in `langGet` at `lang.c:461` → AV. Root: `endscreen.c:1722` calling N64 save path. Side effect: game progress never saved to disk. Two additional dialogs (`g_FilemgrSaveErrorMenuDialog`, `g_FilemgrFileLostMenuDialog`) had identical fn-ptr-title+DANGER+unregistered pattern. Symbolized backtrace: `endscreenPrepare→filemgrSaveOrLoad→pakFindBySerial→menuPushDialog→menuRenderDialog→pdguiHotswapCheck→renderDangerDialog→getDialogTitle→langSafe→langGet` | endscreen.c:1722,1827,1932, filemgr.c:901,1084,1140,1218, pdgui_menu_warning.cpp | **FIXED S190** — Replaced all 3x `filemgrSaveOrLoad(FILEOP_SAVE_GAME_000)` in endscreen.c with `saveSaveAgent(g_GameFile.name)`. Registered 3 filemgr dialogs as noopRender in pdgui_menu_warning.cpp (defense in depth). Side benefit: game saves now actually written to disk on mission completion. |
