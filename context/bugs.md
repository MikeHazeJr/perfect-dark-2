...
| **B-127** | HIGH | WASD held not registering — KBM move axis synthesis in `actionmapPollFrame()` set `.value` but not `.held` flag for ACTION_AXIS_MOVE_X/Y. Movement only worked for one frame (edge-triggered), not continuous. Gamepad stick path correctly set `.held`. | actionmap.cpp:793 | **FIXED S187** — Added `.held` flag update mirroring gamepad pattern |
