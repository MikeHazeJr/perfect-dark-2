...
| B-17 | **Mod stages load wrong maps** — Kakariko selection loads different map, 4 garbage entries at end of stage list | HIGH | S26 | Root cause: `modConfigParseStage()` patches `g_Stages[]` with wrong file IDs. | **STRUCTURALLY FIXED (S32)**: Catalog smart bgdata redirect bypasses `g_Stages[]` patching. Verified on: Paradox ✓, Facility ✓, Temple ✓, + 1 more (S35). GE ammo crate scale issue persists on some maps (see B-13). |
