...
|----|----------|-------------|------|--------|
| **B-128** | MED | Sky tearing / transparent sky tris — "the sky still has tearing or it may be tris that are supposed to be transparent or something rendering wrong." Root cause: sky upper-hemisphere (clouds) rendering in `skyRender()` at line 1244 never calls `gDPSetRenderMode`, inheriting stale `other_mode_l` from the previous frame. If the previous frame ended with `G_RM_AA_XLU_SURF` (sun flares, teleport effects, overexposure), `use_alpha=true` in gfx_pc.cpp:1307 enables `glBlendFunc`, causing sky tris to alpha-blend with the framebuffer instead of overwriting it. Sky is the FIRST geometry drawn each frame (lv.c:1394), so it always draws with last-frame's blend state. Fix: added `gDPSetRenderMode(gdl++, G_RM_OPA_SURF, G_RM_OPA_SURF2)` after `gDPPipeSync` at sky.c:1244 — mirrors working water path at sky.c:827. Vertex alpha is always 0xff (sky.c:189), confirming OPA_SURF is correct. Build verified: sky.c.obj 08:07, PerfectDark.exe 48596071 bytes, clean link. | src/game/sky.c:1244 | **FIXED 2026-04-10** — awaiting visual confirmation on outdoor stage |
...
| **B-130** | HIGH | Theme editor X button and click-outside dismiss do nothing — both the title-bar X button (`!open` after End) and InvisibleButton click-outside overlay fail to close the editor. S197a shipped `&open` to Begin + both exit paths + overlay with SetNextWindowFocus, but Chris's playtest (v0.0.74, confirmed S197a present) showed all three exit paths (X, Close button, click outside) are ineffective. Working hypothesis: z-order — the Settings menu (or other persisted ImGui window) sits above the overlay in z-stack, intercepting outside-area clicks before the InvisibleButton fires. X button may be failing due to focus state or re-show race. | port/fast3d/pdgui_menu_theme_editor.cpp | OPEN (S198) — lifecycle instrumentation deployed (4 sysLogPrintf calls at each exit site); diagnose from next playtest log. Do not fix blind. |
| **B-131** | MED | Post-restart Start button double-fire — after Mission 1 loops back to start (caused by B-129), the first Start press opens the menu and immediately closes it; only a rapid double-press sticks. After the menu is open once, close/reopen works normally. Working hypothesis: residual `menuinputs.start` or stuck consume flag, OR lingering deferred-pop inputctx entry across `mainChangeToStage`. | port/src/inputctx.c, src/game/bondmove.c | OPEN (S198) — confirmed real (S197a is in build); deferred to follow-up session after B-129 lands. |
| **B-126** | HIGH | Silent crash after ~8 minutes in MP — process dies with no error/VEH log. 7 bots + 1 player, well under MAX_MPCHRS. Stack (8MB), VEH, H-7 all verified in build. Added heartbeat logger (every 60s) to lv.c. S191: heartbeat expanded (60s→30s interval; last_chr_idx added; full NET.WATCHDOG peer-state dump via netHeartbeatLog() in net.c; SIGABRT handler logs chr index). Likely cause: -fstack-protector-strong canary smash (SIGABRT bypasses VEH); crashSigabrtHandler registered and writes to log — if silence continues after next repro the abort() path is not triggering either. | lv.c (heartbeat), crash.c (SIGABRT), port/src/net/net.c (netHeartbeatLog) | INVESTIGATING (S191) — richer instrumentation deployed, awaiting next repro |
...
| **B-112** | HIGH | Chr pointer (rbx) corruption in `chraTick` during 31-bot matches — access violation at `chr->hidden` (line ~13444, BEFORE previous canary at ~13477). S191: entry guard added at top of chraTick (before CHRRACE deref); g_ChrLastTickedIndex tracker added (chr.c:chrTick writes slot index before chraTick, clears after); SIGABRT handler (crash.c) reads the index so next crash pinpoints which bot was mid-tick. Root cause still unknown (stack corruption during AI/action-dispatch callee). | src/game/chraction.c, chr.c | PARTIAL (S191) — entry guard closes the pre-canary crash gap; per-chr index tracker enables post-mortem slot ID; awaiting next 31-bot crash log |
| **B-18** | MED | Pink sky on Skedar Ruins — sky renders pink instead of correct color | sky rendering path | OPEN — B-128 fix (sky.c:1244 `gDPSetRenderMode`) may fully or partially resolve this; needs Skedar Ruins playtest to confirm. |
