...
|----|----------|--------|------|--------|
| **B-145** | HIGH | **FIXED-PENDING-PLAYTEST 2026-04-16 (festive-saha)**. GAP-1: Deep Sea co-op next-mission branch in `menutick.c` called `mainChangeToStage()` without `manifestClear(&g_ClientManifest)`. Matches the SP-13 / Bug A crash pattern exactly — stale MP manifest diffed by `manifestMPTransition()` → AV. Fix: `manifestClear(&g_ClientManifest)` inserted immediately before `mainChangeToStage()`. | `src/game/menutick.c:~598` | Online co-op campaign: complete Deep Sea co-op mission with endscreen-driven advance; no crash on stage transition. |
