...
- **Bug C (post-game endscreen partial render)** — scrim + title-bar rectangle render but content body is invisible. Six hypotheses in `scratch/archive/2026-04-13/session-state-endgame-crash.md` §4c. **Instrumented 2026-04-16 (festive-saha)**: `renderMpEndscreen` now emits `LOG_WARNING` on the `Begin=false` early-return (line ~755) with `sf / menuW / menuH / disp` and on the `contentH` clamp (line ~829) with `sf / menuH / padY / raw / min`. Next repro pass should tail `pd.log` for `ENDSCREEN:` to narrow the six hypotheses before any structural change.
