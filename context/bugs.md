...
| B-13 | **GE prop scaling ~10x on mod stages** — Ammo crate oversized on Kakariko | MED | S26 | `model.c` lines 857–858, 883–884 apply `model->scale` but NOT `model->definition->scale`. `modelGetEffectiveScale()` exists and correctly multiplies both but is only used for collision, not rendering. | FIXED (S26): Both rendering paths changed to `modelGetEffectiveScale(model)`. **Needs build test.** |
| B-14 | **START on controller opens/closes pause immediately** — double-fire or input passthrough | MED | S26 | Legacy path (bondmove→ingame.c) opens pause, then ImGui render sees same GamepadStart press via polling and immediately closes it — all in one frame. | FIXED (S26): Frame guard `s_PauseJustOpened` skips close checks on the open frame. Also added pause menu to `pdguiProcessEvent` input consumption. **Needs build test.** |
...
| B-16 | **Back on controller does nothing** in ImGui pause menu | MED | S26 | Pause menu render function never handled `ImGuiKey_GamepadFaceRight` (B button). Input also wasn't consumed — `pdguiProcessEvent` excluded pause menu from the consumption check. | FIXED (S26): B button now navigates back (cancels End Game confirm, or closes pause). Added `pauseActive` to event consumption. **Needs build test.** |
| B-17 | **Mod stages load wrong maps** — Kakariko selection loads different map, 4 garbage entries at end of stage list | HIGH | S26 | Likely stage ID → stagenum mapping mismatch in `g_MpArenas` bonus entries vs `stageGetIndex()` lookup in `stagetable.c`. Also Skedar Ruins has wrong textures on ammo crates/walls. | NEEDS DEEPER DIAGNOSIS |
