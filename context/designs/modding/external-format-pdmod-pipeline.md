...
- Authored mod content and `.pdmod` transport payloads must not contain `.bin` or public `.tsv` files. Engine-native binary data may exist only as an internal cache generated from external source files, and public TSV tables are not valid final source.
...
- UI textures: `ui/<id>/ui.ini` points at `texture.png` or `texture.tga`; optional UI layout source is semantic `layout.json`, not TSV. The runtime loads texture bytes through `fsFileLoad`, so mounted archive entries and loose files use the same path.
- Fonts: `fonts/<id>/font.ini` points at `font.ttf` or `font.otf` for authored UI fonts. Base `.pdfont` extraction exposes ROM bitmap fonts as `glyphs.pgm` plus semantic `font.metrics.json` metric/kerning source instead of `data.bin` or public TSV bridges.
- Language banks: `lang/<id>/lang.ini` points at semantic UTF-8 `strings.json` localization source rather than ripped public TSV rows. `.pdlang` still compiles that source back into the normal 512-entry language-bank runtime table, but the modder-facing file is JSON, not a tabular byte bridge.
...
- Music: `audio/music/<id>.pdsong` carries `music.ini`, standard `sequence.mid`, and semantic `sequence.json` event source. Base extraction inflates the N64 RareZip compressed-MIDI stream and converts it rather than exposing a raw `data.bin` authored payload; final source keeps MIDI plus named cue/event metadata instead of public TSV.
...
Current s7 state: done/current as of 2026-06-11. `modpackPdmodFromFolder()` validates typed `.pdxxx` asset archives plus the compatibility canonical layout before writing, rejects authored `.bin` and public `.tsv`, generates missing commented descriptor and supporting INI templates for the compatibility layout, checks referenced source files and unsafe paths, recognizes scenario-backed `.pdarena` dependencies and `.pdsong` `music_file`/`midi_file`/`track_file` source keys, and surfaces detailed errors to Modding Hub through `modpackPdmodLastError()`. The in-memory writer rejects `.bin` and `.tsv` entries too. The checked-in all-family example now has production-packer proof: `modpackPdmodFromFolder()` packages it and preserves every typed archive byte-for-byte inside `.pdmod` transport. Modding Hub's active pack/import tool no longer exposes the legacy `.pdpack` workflow; it packs `.pdmod` from a folder and imports `.pdmod` through `modmgrInstallArchiveFile()`.

Current s8 state: done/current as of 2026-06-11. Mike's Build log validated boot, legacy `.pd*` runtime registration, `.pdui` auto-emit/reload, and zero-bin installed `.pdmod` discovery. Runtime smoke gates now validate the preferred authored shape both ways: enabled typed `.pdxxx` content as a loose folder mod and the same content wrapped in real `.pdmod` transport, loading GLTF head, OBJ arena, and GLTF skeletal animation sources into modeldef, colmesh, and animation clip payloads through readable generated caches. Public Mods transfer/install closure now uses shared `modmgrValidateArchiveFile()` before install: received `.pdmod` downloads validate root `mod.json`, reject authored `.bin` and public `.tsv`, release-validate nested typed `.pdxxx` archive bytes, install into `mods/installed`, refresh `g_ModRegistry`, and compare archive/folder mods by root `mod.json` digest. The shared validator/import helper update is verified with source guard, all-family workflow verifier, all-family conformance, focused pdmod/Public Mods tests, scoped diff-check, and isolated `s110import` all-target build.
