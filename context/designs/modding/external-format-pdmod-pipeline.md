...
- Base model, scenario, and character-animation extraction now avoids authored `.bin`: `.pdmesh` archives expose standard Wavefront `model.obj` plus `model.mtl` generated from promoted `PD_MODELDEF` display lists; `.pdscenario` archives expose `rooms.obj`, `scenario.mtl`, `tiles.tsv`, `pads.tsv`, and visual OBJ/MTL/TGA payloads, while raw setup/mpsetup/visual word dumps are withheld until decoded catalog-ID tables/graphs replace them; character `.pdanim` archives expose `header.tsv` and `frames.tsv`. Semantic GLTF hierarchy/material export and richer animation-channel export remain quality upgrades, but mesh and scenario closure are no longer byte-table or raw stage-binary fallbacks.
