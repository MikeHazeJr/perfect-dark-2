...
For `.pdmesh` render streams, conformance, offline source verification, and
targeted render-state proof import the same typed schema validator. Schema v2
requires complete unique face coverage, valid render-group ownership, exact
geometry set/clear masks, and per-corner load-time state; the runtime compiler
enforces the same cardinality and ownership before consuming public source.
New producers always emit numeric `pd_schema_version`. Existing public v1
streams remain readable only when they carry the exact historical semantic
schema `pd2.mesh.render.v1`; absent or unknown identity is rejected.
...
| `.pdmesh` | `model.gltf`, `model.glb`, `model.obj`, `model.mtl`, `model.nodes.json`, `model.parts.json`, `model.faces.json`, `model.render.json`, `export_version.txt`, `dependencies/assets/{materials,textures}/*.pdxxx` | One mesh source is required. OBJ may use `model.mtl`; extracted native modeldefs use semantic JSON for hierarchy, part lookup, face matrix bindings, render command order, exact geometry set/clear state, and per-corner source-vertex-load known/value state. Render schema v3 records every ordered `vertex_load` and each triangle's exact `vertex_cache_slots`; one uniquely owned Type-3 `vertex_scope` retains the bounded cache across its opaque/translucent boundary, while one non-Type-3 `vertex_cache_reset` clears it. Load/draw state may differ only when that ordered provenance is exact and the compiler can restore the effective draw state; caller-owned fog is never guessed. Schema v2 retains strict current-state matching. A top-level native display list without an extractor-readable vertex domain remains a valid zero-triangle hierarchy/part source; recursive display lists become strict only after a real vertex domain is admitted. Exporter provenance is diagnostic. Public mesh TSV sidecars are not valid final source. Textures/materials are typed dependencies or embedded/declared by the model source, never loose mesh sidecars. |
