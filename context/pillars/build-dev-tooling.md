...
The Assets tab (2026-06-11) wraps [devtools/pdxxx-asset-tool.ps1](../../devtools/pdxxx-asset-tool.ps1) / [pdxxx-asset-tool.psm1](../../devtools/pdxxx-asset-tool.psm1). It crawls data folders for typed `.pdxxx` archives, filters by type, shows source hints, extracts selected assets into ignored `.pdxxx-dev-extracts/`, and opens the latest output folder. The tool is deliberately extraction-only: it preserves stored archive files, expands nested typed archives only into adjacent inspection folders, and performs no conversion or asset translation.
...
B-372 release-hook fix (2026-05-26): Dev Window v2 Release can reach a successful client link and still fail at the pre-release commit gate. The 2026-05-26 log showed MSYS `/usr/bin/python3.exe` trying to open a doubled `/c/.../perfect_dark-mike/C:/.../tools/asset_native_source_guard.py` path. `.githooks/pre-commit.py` now invokes the c3842 asset-native guard through repository-relative `tools/asset_native_source_guard.py` with cwd set to the repo root, avoiding mixed Windows/MSYS absolute-path resolution. The same pass removed Dev Window v2's automatic `git commit --no-verify` retry, so failed release sync commits stop at the hook instead of bypassing it.
...
S2026-05-21 c3818: `UNRELEASED.md` is the running simplified change list consumed by GitHub release notes. Completion closeout should add short bullets for what changed and why, what was added, and what was upgraded or modified. `release.ps1` warns when the file still looks like the old placeholder notes, and release/pre-build/rebase sync commits mirror Codex memory before staging.
