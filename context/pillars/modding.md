...
- **Weapon behavior belongs in `.pdwpn` authored data** (2026-05-20, c3812). The behavior is not new gameplay invention; it is the existing weapon machinery modularized into the project-owned asset format. `.pdwpn` must be able to express trigger-pulled actions, centisecond fire cadence, custom projectiles, looping fire, hold beams, charge/release, secondary modes, melee, zoom, reticle/overlay/camera effects, and ammo-driven model or screen state.
...
- **Typed `*.pdxxx` archive repair active (2026-05-20, c3812).** Mike corrected c3811's loose descriptor model: a `.pdhead`, `.pdarena`, `.pdanim`, etc. is itself a zip-openable typed asset archive. The archive root carries the readable descriptor (`head.ini`, `arena.ini`, `animation.ini`, etc.) plus authored source assets such as GLTF/GLB/OBJ, textures, audio, TSV, fonts, and supporting INI files as needed. Changing the extension to `.zip` should expose everything required to create a custom version of that asset. `.pdmod` remains only the sharing/Public Mods/online delivery transport wrapper; authored `.bin` payloads remain invalid.
- **`.pdwpn` behavior format added to c3812 scope (2026-05-20).** Weapon archives must carry editable behavior data in addition to model/animation/audio relationships. The first authoring target is `when trigger pulled -> shoot <custom projectile> every <centiseconds>`, with room for rapid fire, sustained beams, charge-and-release, secondary functions, melee, zoom and reticle/overlay/camera effects, and ammo-state presentation such as physical needle slots or numeric ammo screens.
