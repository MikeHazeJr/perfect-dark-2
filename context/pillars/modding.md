...
- **c3814 active status update (2026-05-22).** `c3814-s15` is done for the held/player/AI runtime adapter, including the B-363 MKB weapon-switch/function-HUD fix and the second-pass Magsec/Falcon primary/secondary text fix. `c3814-s24` is done for the modular graph-editor decision: one `.pdweapon` per weapon, `primary`/`secondary` subgraphs inside `behavior.graph.json`, explicit shared context variables for owner/damage/detonator/deployed-entity/targeting/hacking state, compiler preservation/hashing, base-emitter output, and Modding Hub editor controls/seeds. `c3814-s25` is done for the in-game Blueprint-style node editor in the Weapons Template flow, including the in-place tabbed `Create Weapon Mod` drill-in view and primary/secondary graph tabs populated from the selected template graph. Current active next remains `c3814-s16` for deeper `.pdprojectile` runtime execution.
