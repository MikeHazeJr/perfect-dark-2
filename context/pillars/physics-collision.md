...
- **Scenario collision override and deterministic navmesh source contract done under c3841.** `.pdscenario` authoring uses the same textured `scene.glb`/`scene.gltf` that opens in Blender/3DS Max and feeds catalog/runtime metadata. Collision uses an explicit `collision_file` override when present, otherwise falls back to `modAssetCompilerBuildColmesh()` from the scene source and records `_meta/generated-collision.json`. Bot navigation source inputs are now explicit in `navigation.ini` and `_meta/generated-navmesh.json`, covering walk, jump, drop, wall traversal, and ceiling traversal capability classes.
