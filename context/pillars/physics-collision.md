...
- **B-350 Airborne side-entry through overhead blockers -- BUILD/TEST VERIFIED 2026-05-19; pending CI Training playtest.** Follow-up to c038/B-339 after Mike's playtest showed top/bottom blocking improved but side-entry during a jump could still pass through overhead blockers. Root cause was movement order: horizontal movement applied first, then `bwalkUpdateVertical()` only swept `move=(0, verticalDelta, 0)`. `bondwalk.c` now runs an upward diagonal `capsuleSweep()` from `bondprevpos` to current X/Z plus `verticalDelta` before the vertical-only sweep, rolls X/Z back to the safe fraction on any hit, and clamps/kills upward movement on ceiling-class hits. `tests/test_jump_two_stage_design.cpp` now includes side-wall rendered-triangle fixture coverage. Verification: queued isolated `b350side` tests target PASS, focused `[physics][jump]` PASS (99 assertions / 4 cases), and all-target build PASS.
- **B-367 Airborne lateral wall/ceiling/corner clamp -- BUILD/TEST VERIFIED 2026-05-22; pending Mike playtest.** Follow-up after Mike reported the jumping player still clips into walls, ceilings, or corners. The remaining movement-order gap was horizontal movement committing through the legacy cylinder path before the vertical repair code ran, so apex/descent or same-frame wall/corner contact could enter rendered-only geometry. `bondwalk.c` now runs `bwalkClampAirborneLateralEntry()` immediately after horizontal movement and before speed correction when the player is airborne. The helper performs a horizontal collision-owned capsule mesh sweep from `bondprevpos`, rolls X/Z back to a safe fraction on wall/ceiling/corner hits, ignores floor-class hits, and leaves the existing upward diagonal B-350 sweep in place. Verification: focused `[physics][jump]` PASS (105 assertions / 4 cases) and isolated `jump367` all-target build PASS.
...
- **Skedar swarm jump/surface parity + stress hardening -- BUILD/TEST/SMOKE VERIFIED 2026-05-19; pending playtest.** Benchmark-local helper drives Skedar leap requests and wall/ceiling surface-transition requests for both CPU and GPU swarm paths. The 2026-05-19 stress follow-up (B-352) hardens the high-count lifecycle after Mike's GPU 128 -> 256 crash and CPU 4096 renderer fatal: GPU slot-keyed readback/floor state invalidates before mass-free, death cleanup no longer writes death-union fields for non-death actions, spawn candidates snap/reject against real non-`GEOFLAG_DIE` floor, Skedar jump starts are phased/frame-budgeted, and <=256-bot GPU readback fences get a short bounded wait to reduce `active=0` jitter. B-353 then closed the intermittent runner crash by requiring every Swarm slot to prove a live prop/backlink before teardown, death polling, movement intent, CPU/GPU loops, or GPU handoff; stale slots are cleared/refilled instead of being passed to `chrRemove()`. Verified with focused `[testscenarios][static][b352]`, queued isolated `skstress`/`sksmoke`/`sklive` all-target builds, and normal `swarm_gpu_b352_stress_smoke` runner PASS twice to 256 with 24/24 assertions and exit 0. Normal Combat Sim bot jumping remains the D2c toggle above; this slice stays benchmark-local.
...
- **Scenario collision/tile source contract active under c3841/c3844.** `.pdscenario` authoring uses the same textured `scene.glb`/`scene.gltf` that opens in Blender/3DS Max and feeds catalog/runtime metadata. Collision uses an explicit `collision_file` override when present, otherwise falls back to `modAssetCompilerBuildColmesh()` from the scene source and records `_meta/generated-collision.json`. As of 2026-05-28, generated `scene.glb` carries `_PD_ROOM` vertex metadata and `tilesReset()` can compile a runtime tile cache from the source-derived Scenario colmesh before legacy tile fallback; Chicago verified 107 rooms / 6,661 source tiles. Bot navigation source inputs are explicit in `navigation.ini` and `_meta/generated-navmesh.json`, covering walk, jump, drop, wall traversal, and ceiling traversal capability classes.
