...
- **2026-08-08 `.pdeffect` v2 production consumers (T-ASSETS-019/B-1004)**: complete public explosion, spark, and smoke libraries now transactionally override the real 26/27/23 native row tables, including typed SFX resolution and explosion-to-smoke ownership. Exact growable typed dependency enumeration runs before catalog activation for standalone, base, nested weapon, mod, and network-scanned sources; missing, empty, null, malformed, aliased, wrong-node, wrong-type, cyclic, or unavailable dependencies reject without native fallback. T-ASSETS-037 closes nested discovery/preflight omission above 64 and T-ASSETS-033 implements exact reverse child invalidation. T019 remains partial for the generic v1 scheduler and consumers (`T-ASSETS-032/034/035/036`) and live edited-source proof (`V-009`).
- **2026-08-08 typed v1 effect scheduler foundation (T-ASSETS-032/B-1011, verified partial)**: every accepted v1 opcode maps to one permanent typed dispatch slot; activation rejects incomplete, corrupt, dependency-violating, or unmapped schedules; and dispatch preflights the complete handler table before a deterministic begin/commit/rollback transaction. Final combined Wave9 automation passes T032 67/6, full 52,721/943, conformance, and the native-source guard. No production caller installs or invokes the handlers yet, so this is not gameplay execution evidence: `T-ASSETS-034/035/036` still own gameplay/audio, renderer/presentation, and timeline/context/target/lifetime consumers.
