...
- **2026-05-28 scenario navigation public-source runtime (`B-384`; semantic JSON updated 2026-06-07, B-785)**: `.pdscenario` now requires `navigation/waypoints.json`, `navigation/waygroups.json`, and `navigation/covers.json`, and stage load compiles those archive-member FileProvider sources into runtime waypoint/group/cover tables beside `pads.json`. These JSON rows are local scenario structure, not asset references; asset references in public payloads still use actual catalog IDs only. Setup/tiles, room-portal source-derived runtime, and mission/level graph behavior remain active under `c3844-s1`.
