...
- **2026-08-10 through 2026-08-12 edited Needler production proof (T-ASSETS-025/V-009; B-1018 through B-1042)**: direct `MP3_*` file references emit exact editable `.pdvoice` sources; the creator and strict parser agree; custom activation installs and clears a loader-owned public adapter; and specific-spawn setup preserves runtime identity. Ordinary gameplay now proves secondary collision, catalog audio, all-consumer effect dispatch, restart, Mod Manager teardown/remount, valid replacement plus rejected-candidate rollback, overlapping-owner `2->1->0`, and isolated real-peer transfer from a client with no local package. The final peer receipt passes 87/87 and both peers exit cleanly. T-ASSETS-025 is implemented; V-009 remains partial only because retained images are overexposed and do not support a readable pixel-level comparison.
- **2026-08-11 transactional live effect replacement and overlapping ownership (V-009; B-1027/B-1028/B-1029/B-1030)**: received typed replacement now invalidates the selected root and dependents, restores retired catalog metadata without stale active refs/payloads, carries recognized rejection as a sticky recursive scan result, defers root reload until the PDCA destination is committed or rolled back, and resolves manifest component identity through the mod registry. The replacement client passes 29/29. A second ordinary-client receipt passes 40/40 while an explicit second weapon owner releases weapon/effect/SFX `2->1`, then real Mod Manager disable retires the remaining owner and complete closure `1->0`. B-1030 snapshots ImGui style scope across mutable apply-state transitions. Full 54,992/998 plus native-source guard, scanner selftest, 27-family conformance, and diff check pass. Real-peer distribution now passes 87/87; V-009 remains partial only for readable visual comparison.
