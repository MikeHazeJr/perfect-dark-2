...
to all 34 previously narrow catalog fields and their audited walker, weapon,
network, runtime, provider, and compiler mirrors. Checked copy/join operations
never return a plausible prefix. Network candidates preflight the same
canonical path-key inventory as local/typed scanning and restore or unregister
the row before dependency commit on failure. Semantic metadata capacities did
not change. The isolated all-target/test builds, 399 focused assertions,
native-source guard, conformance selftest, and 28-root/52-recursive all-family
example conformance pass. The Workbench item remains partial pending real
...
B-1006 now source-freezes the missing I/O transaction. PDCA receive writes and
file-syncs a complete unique sibling stage, compares Windows-normalized
collision identities without lowercasing authored output, publishes by rename,
and restores a pre-existing destination on open/write/publish failure. It also
rejects ADS, device, dot/empty, wildcard, traversal, and case/separator alias
members; cleans only matching abandoned stages; restores an unambiguous crash
backup; and blocks when both destination and backup exist. The combined test
receipt is pending the joint T019/T020 source fingerprint. Real three-ingress
installed-client catalog/FileProvider/runtime identity remains the explicit
T-CATALOG-003 residual.

B-1012 closes the later catalog-admission filesystem gap. Received PDCA
extraction now retains the prior destination backup after candidate publication
and commits only when the typed scanner admits the candidate. Rejection removes
the candidate, restores the exact prior install, and leaves the received count
unchanged. A dynamic ordinary-client fixture builds loose, nested `.pdmod`, and
raw PDCA `.pdgamemode` sources at exact 127/128/1023/1024 FileProvider path
lengths, routes them through the real scanners/receive handlers, and compares
catalog and runtime paths byte-for-byte. The authoritative client passes 24/24
with nine exact identities and three complete rejected-state checks; focused
3,576/13, full 53,264/949, guard, and 28-root/52-recursive all-family
conformance also pass. This is still partial because a mixed-validity,
multi-descriptor
...
aggregate typed-asset reference count. Every non-bundled catalog row carries a
single 0/1 stage-owner ledger maintained by `catalogLoadStageAsset` and
`catalogReleaseStageAsset`; repeated acquisition for the same stage is a
truthful no-op. `catalogComputeStageDiff` compares the destination category
only against ledger-owned rows, never every loaded payload. As a result, a
normal stage transition cannot release theme/UI/font/audio, editor, screen,
manifest, network, or other explicit references it did not acquire. Generic
typed lifecycle APIs remain the owner boundary for those subsystems, while the
stage API composes over the same production load/unload implementation.

The source-frozen client/updater/test build, focused ownership band (5 cases /
74 assertions), full suite (901 cases / 48,165 assertions), and source guard
pass. Production shutdown now releases the active theme transaction before
theme/backend teardown (B-1001). The final source-frozen ordinary-client
CI-to-credits smoke passes 33/33: all five active theme rows survive at `ref=1
stage_ref=0`, emit five shutdown-specific pre-release markers, and balance to
zero. T-CATALOG-002 is `implemented`; broader typed-family live stress remains
a validation gate.
...
- **2026-08-08 through 2026-08-10 `.pdeffect` production consumers (T-ASSETS-019/B-1004)**: complete public explosion, spark, and smoke libraries transactionally override the real 26/27/23 native rows, including typed SFX and explosion-to-smoke ownership. Generic v1 programs now dispatch every accepted gameplay/audio/presentation/timeline/context/target/attachment/priority/lifetime field through T-ASSETS-032/034/035/036. Exact dependencies preflight before activation; unsupported or unavailable source rejects without native fallback; T-ASSETS-033/037 supply reverse invalidation and growable nested discovery. Live edited-source/peer proof remains V-009.
- **2026-08-10 typed v1 scheduler and consumer transaction (T-ASSETS-032/034/035/036; B-1011/B-1014/B-1015/B-1016)**: every accepted opcode maps to a strict typed schedule. A growable production instance owner resolves topology and contexts, samples intensity, and runs all gameplay/audio/presentation observers through begin/stage/prepare/commit with reverse rollback. Production output covers explosion, spark, smoke, typed SFX, distinct source-backed particles, screen textures, world decals, room lights/halos, and beams. The canonical 67-row schema rejects unknown, mistyped, unsupported, numeric-audio, or behaviorally inert fields at activation. Final Wave11 automation passes T034 149/11, T035 184/8, T036 525/13, full 54,175/980, conformance, guard, diff check, and a stable fingerprint.
- **2026-08-08 growable weapon nested ingress and runtime capacity (T-ASSETS-037/B-1010; T-ASSETS-022/B-992)**: weapon UI/audio/animation/effect enumeration and complete transactional preflight now grow on PC instead of rejecting after 64 entries. The scanner reserves the full direct plus effect-owned edge closure before publication and reverse-rolls back only rows/edges created by the transaction. Recursive conformance passes a generated public-source fixture with 70 SFX archives and two individually valid effects carrying 35 typed audio dependencies each: 72 direct nested archives, 70 aggregate effect-owned edges, 1 root, and 73 checked archives. The original production client ingress proof passes 7/7 with harness 2/2 and clean rollback. B-992 separately expands the private 11-bit sound domain to all 502 remaining identities, prevents base-only weapons from spending custom pairs, and expands the paired weapon domain safely within the existing 64-bit save filter. Final frozen verification passes client/updater/tests compile, 4,600/85 focused, native-source guard, and an 11/11 installed-client smoke allocating a new weapon after the base walk and starting all 70 SFX through the production catalog/audio route under a matching 38-file source fingerprint.
- **2026-08-10 edited Needler adapter milestone (T-ASSETS-025/V-009; B-1018 through B-1022)**: direct `MP3_*` file references now emit exact editable `.pdvoice` sources; the Needler creator and conformance schema agree with the strict effect parser; custom weapon activation installs and clears a loader-owned runtime adapter from public `weapon.ini` plus compiled held graphs; and specific-spawn setup preserves the catalog row's `runtime_index` instead of re-deriving through the MP index. Frozen automation passes 54,777/983, all-family/Needler conformance, and the source guard. The installed-client smoke passes 43/43 with runtime 86, file 2016, nested effect ingest, exact first-person source load, and generated-model submission. This is partial validation only: recognizable held-model fidelity, actual effect/audio/presentation output, restart, replacement/rollback, and real-peer lifecycle proof remain open.
- **2026-08-11 Needler edited-value production proof (V-009)**: two deterministic public `.pdeffect` variants pass separate 46/46 ordinary-client runs through the same secondary-projectile collision, catalog audio, gameplay/presentation transaction, and GBI renderer. Pink `1.000,0.400,0.800,1.000` and cyan `0.000,1.000,1.000,1.000` remain exact from authored source through prepared and final material color. The source/test fingerprint is stable and 54,876/987 tests plus guard/conformance pass. Retained images are overexposed, so V-009 remains partial for readable visual comparison, replacement/rollback, second-owner, and real-peer validation.
- **2026-08-11 transactional live effect replacement and overlapping ownership (V-009; B-1027/B-1028/B-1029/B-1030)**: received typed replacement now invalidates the selected root and dependents, restores retired catalog metadata without stale active refs/payloads, carries recognized rejection as a sticky recursive scan result, defers root reload until the PDCA destination is committed or rolled back, and resolves manifest component identity through the mod registry. The replacement client passes 29/29. A second ordinary-client receipt passes 40/40 while an explicit second weapon owner releases weapon/effect/SFX `2->1`, then real Mod Manager disable retires the remaining owner and complete closure `1->0`. B-1030 snapshots ImGui style scope across mutable apply-state transitions. Full 54,992/998 plus native-source guard, scanner selftest, 27-family conformance, and diff check pass. V-009 remains partial only for readable visual comparison and real-peer distribution.
- **2026-08-08 `.pdeffect` failure/lifecycle closure (T-ASSETS-020/B-1005/B-1008)**: selected nonempty refs now fail instead of substituting an OG value; nested failures reject their parent; complete-source owner identities prevent graph-equal/timeline-different collisions; and a growable iterative dependency plan preflights exact types, cycles, diamonds, and arbitrary closure size before ordered activation/reverse rollback. Effect/weapon disable and reset retire parent-first closures outside the catalog mutex, full reset clears the executor plus all edges, and mod reset preserves bundled edges. `T-ASSETS-033` now supplies exact reverse invalidation when a loaded child dependency is disabled; `T-CATALOG-004` supplies family-generic teardown.
- **2026-08-08 all-family lifecycle transaction (T-CATALOG-004/B-1009)**: disable, mod reset, full reset, and reinitialization now snapshot every selected typed row and preflight every root before the first mutation. A growable planner orders parents before shared children; family teardown covers provider bytes, generated modeldefs/collision/animation, language and generic public-source bindings, weapon/effect owners, and stage ownership outside the catalog mutex. Mod reset preserves bundled rows/edges/adapters; full reset clears all process adapters, edges, and private slots before identity reuse. Mod rescan rebuilds runtime-ID/body/head pointer caches. Final combined automation passes 198/3 focused and 52,721/943 full-suite evidence; ordinary installed-client every-family toggle/restart and concurrent-owner validation remain open. T-ASSETS-033 reverse child invalidation is implemented with separate live-stress validation remaining.
- **2026-08-08 reverse typed dependency invalidation (T-ASSETS-033/B-1013, implemented)**: typed root loads, retains, releases, and stage ownership now publish to a growable exact owner ledger. Child disable/replacement preflights every active root closure, including cycles and type truth, before retiring exact overlapping/diamond root counts and their effect executor/cache adapters. Invalid roots stay pending and unreachable; re-enable or completed replacement reloads only through the ordinary complete typed activation transaction. Successful `.pdeffect` replacement prunes stale prior edges to its exact new public source, while rejection restores and reloads the prior row/edge set. Mod reset drops removed mod owners while preserving bundled pending owners; full reset clears the ledger. A combined installed-client run exposed and preserved a configured voice-backed native sound-token startup regression; the narrow executor correction keeps ordinary voice/music rejected. The authoritative rerun passes installed-client 24/24, focused catalog 3,576/13, complete suite 53,264/949, conformance, and guard. Live child toggle/replacement, real-peer replacement, and concurrent-owner stress remain open, so this is not validated.
