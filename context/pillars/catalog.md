...
**.pdbase format (in flight).** External archive format for base-game content data, paralleling `.pdmod` for mod content. The loader scaffold ([port/src/loader_pdbase.c](../../port/src/loader_pdbase.c), 61 lines) and header ([port/include/loader_pdbase.h](../../port/include/loader_pdbase.h)) exist but the directory walk and decode are not yet implemented. F11+ (per the catalog full-pipeline weapons design) moves Layer A weapon data into `.pdbase` archives, populates `ext.weapon.pdbase_path` / `pdbase_offset` / `pdbase_size` on catalog entries, and retires the weapon parts of Layer A.
