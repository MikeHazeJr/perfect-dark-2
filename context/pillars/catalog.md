...
- **2026-08-11 transactional live effect replacement (V-009; B-1027/B-1028/B-1029)**: received typed replacement now invalidates the selected root and dependents, restores retired catalog metadata without stale active refs/payloads, carries recognized rejection as a sticky recursive scan result, defers root reload until the PDCA destination is committed or rolled back, and resolves manifest component identity through the mod registry. The final installed client passes 29/29: pink baseline, valid cyan replacement, invalid same-slot rejection, filesystem rollback, pending-root reload, and restored cyan gameplay all occur in one process. The smoke wrapper also filters auxiliary pipeline output and writes the JSON receipt. Full 54,972/996 plus native-source guard and 27-family conformance pass. V-009 remains partial only for readable visual comparison, a second overlapping owner, and real-peer distribution.
