...
**.pdbase format (F11 shipped 2026-04-30).** External JSON archive format for base-game content data, paralleling `.pdmod` for mod content. F10 (S484) shipped the loader scaffold ([port/src/loader_pdbase.c](../../port/src/loader_pdbase.c), 61 lines, no-op decoder); F11 (this slice) ships the first generated archive [base/weapons.pdbase](../../base/weapons.pdbase) (~12.8k lines, 86 weapon records + 110 animation records) authored via [devtools/extract_weapons_pdbase.py](../../devtools/extract_weapons_pdbase.py). The extractor parses `src/game/invitems.c` + `src/game/botinv.c`, resolves `#define` constants from `constants.h` + `gunscript.h`, decodes `gunscript_*` / `gunviscmd_*` macro calls into JSON opcode arrays, and inlines sub-records (functions, ammos, aimsettings, noise, recoil, gunviscmds, partvis) per design Section C. Per Mike's 2026-04-30 directive, Path B (data-driven animations) was chosen over Path A (named C symbols) so that animations can later get an IK evaluator without churning the data layer again. Symbolic enum values (ANIM_*, SFX_*, FILE_*, L_GUN_*, MODELPART_*) are preserved as JSON strings; numeric flag-bitfields are integers. Structure pinned by [tests/test_loader_pdbase_scan.cpp](../../tests/test_loader_pdbase_scan.cpp) ([catalog-mgr-weapon][s484][f11], 8 cases / 35 assertions). F12 wires the C-side JSON parser + opcode decoder + manager population; F13 retires Layer A weapon data, animations, and supporting records.
