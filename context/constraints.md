...
- **Catalog identifiers at all interface boundaries**: All asset references at interface boundaries (wire protocol, save files, public APIs) must use catalog identifiers (string IDs or session catalog net_hash values), never raw N64 array indices (bodynum, headnum, filenum, stagenum, texnum, animnum). The session catalog translates between wire hashes and local runtime indices — no boundary-crossing code may use raw indices directly. Added S90 (bot config transmission fix; asset reference audit ~180 sites, 20 patterns).
