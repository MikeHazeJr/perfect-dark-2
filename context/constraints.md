...
- **ENet protocol version**: Must match across clients. Currently **v32** (v27: all net_hash removed from wire; v28: SVC_BOT_AUTHORITY + CLC_BOT_MOVE; v29: room networking R-3; v30: weapon identity uses catalog session refs u16; v31: SVC_PROP_SPAWN modelnum uses catalog session refs, bot body/head decode uses runtime_index directly; v32: scenario identity uses catalog ID string on wire — CLC_LOBBY_START, SVC_STAGE_START, server query). No integer asset identity may appear on the wire (weapons and models use session catalog u16 refs, scenario uses catalog ID string). **Next bump**: remove chrslots (B-12 Phase 3).
