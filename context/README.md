...
> **Last updated**: 2026-04-17, S307 — **Forge level editor F0 Foundation** (tender-sutherland worktree, NOT yet merged to dev): in-game editor mode toggle (Normal ↔ Freefly Dr. Carroll camera), HUD shell, main-menu "Forge" entry hard-wired to CI Training, `MOVEMODE_CUTSCENE` hijack so the player chr freezes during freefly while the freefly tick overrides `prop->pos + vv_theta + vv_verta`, five new `ACTION_FORGE_*` actions (F7 toggle / E ascend / Q descend / LSHIFT boost / LCTRL precision), session-exit watchdog. Single commit `8d412cc1` in `claude/tender-sutherland-536de7`. Worktree intentionally parallel with S306's UI work on `dev` per Mike's instruction. Build clean: PerfectDark.exe 51,780,176 / PerfectDarkServer.exe 22,840,512. Prior: S305 — **Playtest batch**: P0 content-inset on main menu / Settings / CI redirect / Cinema (no more edge-to-edge bleed into nineslice chrome); settings persistence fix (config keys registered after `configInit` are now replayed via pending-value stash; mod themes apply at startup not just built-in palettes); Nine-Slice → Menu Style rename; Theme Editor Save button docked + themes list auto-refresh on Save-as-Mod; Menu Style tool preview scales UP for small images + Advanced controls collapsed; **Font Import as Mod** — new `pdgui_font_mod.*` scans `mods/Fonts/<slug>/*.ttf`, `Video.FontId` pd.ini key, Settings → Video Font dropdown with "(restart required)"; **Theme Bundling** plumbing — `theme.json` gained `"menuStyle"` + `"font"` keys so one theme activation swaps chrome + font + palette; per-agent settings design doc in `context/designs/theme-bundle-and-per-agent-settings-2026-04-16.md`. Prior: S304 — **CRITICAL menu pool slot leak fix** (focused-roentgen): B-160 — darkened/dead main menu on reopen caused by `menuPopDialog` underflow dropping the pool release; three-layer defense-in-depth fix (underflow auto-releases leaked slots, new per-frame `menuPoolConsistencyCheck` watchdog in `menuTick`, main menu renderer migrated to S300 pool-owned ctx). `src/game/menu.c`, `src/game/menutick.c`, `src/include/game/menu.h`, `port/fast3d/pdgui_menu_mainmenu.cpp`. Prior: S303 — **Campaign/Co-Op/Counter-Op full game loop sweep** (reverent-chatelet): end-to-end trace + fixes for Campaign solo + networked Co-Op + Counter-Op loops, manifest pipeline between missions, starting weapons, menu bookends. 7 fixes landed. New `GAMELOOP.{CAMPAIGN,COOP,COUNTEROP,MANIFEST,WEAPON}:` log taxonomy across `menutick.c`, `mainmenu.c`, `endscreen.c`, `playerreset.c`, `pdmain.c`, `net.c`, `netmsg.c`, `pdgui_bridge.c`. Generalized S298 Deep Sea `manifestClear` pattern to MPENDSCREEN restart/exit + COOPCONTINUE exit in `menutick.c`. Counter-Op silent antiplayernum=1 fallback now WARNING-logged. Stale MP manifest leak into SP transition now WARNING-logged. `endscreenPushAnti` `g_Vars.bond` NULL guard. Co-op 1-pad telefrag now auditable via log. Findings: `context/scratch/game-loop-sweep-2026-04-16.md`. Prior: S300 — **Menu pool ctx migration + SP-14 + manifest audit** (peaceful-aryabhata): 10 ImGui renderers migrated off `s_*PushedCtx` bools to pool-owned ctx (`menupoolAcquire/ReleaseDialog`); `menupoolAcquire` now attaches ctx to already-active slots so `menuPushDialog`'s pre-acquire doesn't block the renderer's IsWindowAppearing; three new `MENU_TYPE_*` entries for stacking mpsettings sub-dialogs; `netDisconnect`+`netStartServer` defensively reset `g_NetMatchRoomId`/`g_NetCounterOpClientId` (SP-14 close); manifest diagnostics added. Prior: S302 **Spawn pool robustness** (quirky-mendeleev): tiered selection cascade (T1 optimal → T2 cycled → T3 reused → T4 last-resort), `SPAWN.TIER:` logging, relaxed `playerTrySelectPoolSpawn` gate. S301 **Comprehensive diagnostic instrumentation** (confident-chaum): new `port/src/crashbreadcrumb.{h,c}`, `ENDSCREEN.DIAG:` / `CHR.DIAG:` / `MATCHSTART.DIAG:` / `AUDIO.DIAG:` / `CRASH.DIAG:` tags. S299 Input Authority Phase 2 (menu pool single-instance discipline, `port/src/menupool.{h,c}`, dialogdef→type registry, `menupoolReleaseAll` wired into force-close sites, trusting-banach). S298 S297 follow-up batch (content-inset wired into endscreen + pause menu, Theme Editor + Room Start Match docked footer, menutick/endscreen SP-1 hardening, FIX-B.1 deep manifest scanner, spawn pool reservation + wall-probe + neighbour-room residuals, stoic-proskuriakova); S297 playtest triage (textbox leak + chrome mod visibility, silly-jepsen, B-154/B-155/B-156) + UI polish drop (Chrome tool docked footer + sidebar preview, room member team-sort + colored rows + local highlight, content-inset API + title-bar procedural samples, elegant-mahavira); S296 input/menu fixes, S295 collision/match-pipeline, Room max-bot/team hardening (S292), Nine-Slice Chrome creator + transforms (S288–S290), Select Tunes fix (S291), Mod Apply UX/hardening (S284–S287), Skin Editor fidelity + Dev Window v2 lock (S266–S273), gameplay audit + Counter-Op wire (S262–S265). Earlier: S258 context organization.
...
**Active log** ([session-log.md](session-log.md)): sessions **S241–S307** (rolling window). **Next tier** ([_archive/session-log-archive-S240-and-older.md](_archive/session-log-archive-S240-and-older.md)): S240 through S157. **S1–S119** chunked files in [_archive/sessions/](_archive/sessions/). Map: [INDEX.md](INDEX.md).

| Sessions | Period | Focus |
|----------|--------|-------|
| S307 | 2026-04-17 | **Forge level editor F0 Foundation** (tender-sutherland worktree): in-game level editor mode toggle (Normal ↔ Freefly Dr. Carroll), 6DOF freefly camera (no collision), HUD shell with mode badge + reticle + camera readout + placeholder catalog panel + controls reminder, main-menu "Forge" entry hard-wired to CI Training, `MOVEMODE_CUTSCENE` hijack pattern for the player chr during freefly, session-exit watchdog on stage transitions away from gameplay. New module `src/{game,include/game}/forgemode.{c,h}`; UI shims `port/{include,fast3d}/pdgui_forge*.{h,cpp}` + `pdgui_menu_forge.cpp`; five new `ACTION_FORGE_*` action map entries (F7 toggle, E ascend, Q descend, LSHIFT boost, LCTRL precision); forgeTick wired into `mainTick` between `playermgrShuffle` and the per-player loop; HUD wired into `pdgui_backend.cpp` after `pdguiGameOverRender`. Single commit `8d412cc1` in worktree, NOT merged to dev yet — Mike requested parallel coexistence with S306 UI work until F0 is verified in-game. Build clean: PerfectDark.exe 51,780,176 / PerfectDarkServer.exe 22,840,512. |
