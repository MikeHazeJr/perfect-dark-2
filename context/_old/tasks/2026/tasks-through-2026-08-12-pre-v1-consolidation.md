...
## 1.0 release contract (D-004)

Mike approved the stable base-game plus Theater definition of 1.0. The release
requires complete production implementation and parity for every shipped
base-game mode and every accepted base-game graph surface. Named native C
routines may remain as explicit graph-selected modules, but inert graph data,
hidden bypasses, placeholders, and fallback around authoritative graph source
fail the release gate.

`T-THEATER-001` owns a real Campaign and Combat Simulator replay format and
lifecycle. The existing v1 `.pdth` path records only live spectator participant
snapshots, does not emit match configuration, cannot reconstruct the stage or
world, has no pause/seek, and cannot record an ordinary local Campaign or host
match without a spectator source. `V-011` requires record, process restart,
list, load, coherent playback, camera control, corruption handling, and clean
menu return for Campaign and Combat Simulator.

`T-MODDING-008`/`V-012` replace the Needler as the 1.0 sample-mod requirement.
Ship a deliberately small graph-backed base-gun variant with either a
three-round burst or alternate projectile, and prove its permanent catalog
identity is enabled and usable in Match Setup custom weapon slots through
save/reload, local gameplay, and listen-host synchronization. Needler-specific
polish is post-1.0, while B-1054/B-1055/B-1056 visual evidence remains a
gameplay-capture regression gate.

`V-010` is the complete base-game and graph-parity validation gate, and
`T-RELEASE-001` is the canonical release umbrella. Forge map creation/map
variants, Studio, broad Modding Hub/packer/creator tooling, and V-007 are
post-1.0. `T-MODINFRASTRUCTURE-003` remains a 1.0 correctness gate because a
clean base release must separate bundled AllInOne content into optional typed
mods rather than silently treating it as base-game authority.
...
records have complete-source activation owners, and recursive typed closures
preflight cycles/types/capacity before mutation. Disable/mod/full reset now
...
rollback were the two remaining Wave C fronts. B-1006 now source-freezes the
rollback implementation: received archives write into a unique sibling stage,
sync every member, preserve authored case while rejecting Windows path aliases,
publish by rename, restore an existing destination on failure, clean only
hash-matched abandoned stages, and block ambiguous crash backups. Combined
tests are pending the joint T019/T020 fingerprint. Real installed-client
