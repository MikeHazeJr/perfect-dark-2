...
6. **BLOCKER 4 (.pdmod packaging)**: After authoring, no UI tool packs loose `.pdwpn` + `.pdmesh` files into a `.pdmod`. Modder must hand-write `mod.json` and use 7-Zip -- but they will miss the EOCD comment mirror, so Windows Explorer Property Handler won't surface fields. The `modpackExport` UI writes `.pdpack` (different format), not `.pdmod`.
7. **BLOCKER 5 (load path)**: Even with a correctly packed `.pdmod`, `.pdmesh` files inside archive mods may not register. Per `pillars/modding.md:160-164`: ".pdmod cannot deliver INI-based components." The INI scanner uses `fopen / stat / opendir`, expecting on-disk files; archive-mounted `.pdmesh` files would not appear in the catalog. Folder-mods only for non-JSON-content components today.
