...
|------|----------|--------|--------|
| **D5 Phase 3 -- Remaining menu screens** | HIGH | IN PROGRESS | Full audit complete (S188): 254 dialogs found, 79 screens across 11 batches, ~13-18 sessions. Plan in `context/designs/menu-replacement-plan.md`. No split-screen (2P cancelled). CI Options absorbed into unified Settings. **Batch 0 DONE (S192)**: pdgui_layout primitive (docked action bar + popup scrim), pdguiModelPreview generalized to CHARACTER/WEAPON/VEHICLE/PROP, Mission Select Start Mission docked, Mission Difficulty dialog text-missing regression fixed, Challenges Accept Challenge docked. **Batch 1 DONE (S193)**: seven "low-hanging fruit" dialogs — ExitGame (literal-text fix), PdModeSettings (slider support in fallback), MpEndGame (scrim upgrade), and four filemgr pak-era placeholders redirecting to Agent Select.  1080p scaling baseline flipped in same session (foundation-layer fix). **Batch 2 DONE (S194)**: Co-op / Counter-Op flow — four dialogs, shared impl renderer, +701 lines. **Batches 3, 4, 8, 11, 12 DONE (S195)**: CI Options → unified Settings redirect (9 dialogs) + Cheats (9 dialogs with typed-dialog primitive extended: MENUITEMTYPE_CHECKBOX / DROPDOWN / MARQUEE + function-pointer label callbacks matching legacy `menuResolveText()` contract) + MP Pause & In-Game (6 dialogs) + MP Player Config & Stats (5 dialogs) + Music & Misc (4 dialogs) = 27 dialogs across 5 batches.  2 files touched: pdgui_menu_mainmenu.cpp (2743→3063, +320) and pdgui_menu_warning.cpp (749→1105, +356).  Client 48,739,743 bytes (+23,685 vs S194), server 22,786,384 bytes (unchanged).  S193b server linker guard intact.  **Batches 5, 6, 7, 10 PENDING** — remaining work needs MENUITEMTYPE_LIST / PLAYERSTATS / RANKING / CAROUSEL primitive extensions (next highest-leverage item for S196).  Full per-batch audit in `context/scratch/s195-report.txt`. |
