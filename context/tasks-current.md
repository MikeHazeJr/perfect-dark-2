...
| **B-36 Client crash after skyReset** (S63) | **CODED (S63)** — NULL check in musicIsAnyPlayerInAmbientRoom, double PRIMARYTRACK() fixed, logging added. Both files compile. Needs full client build + playtest on stage with ambient music (MBR, Maian SOS, Skedar Ruins). |
