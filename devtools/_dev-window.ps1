...
#   2. Smart builds: incremental by default, staleness detection, Shift+Click clean
#   3. Visual layout redesign: cleaner spacing/grouping
#   4. Settings: ._dev-window-settings.json (underscore prefix)
#   5. SYNC RULE: cmake flags match build-headless.ps1
#
# PowerShell 5.1 compatible. WinForms.
# ============================================================================
...
# TEMP/TMP fallback for sandbox environments (cc1.exe needs writable temp)
...
# ============================================================================
# Section 5: Settings persistence (._dev-window-settings.json)
# ============================================================================
...
# Smart staleness detection: returns $true if CMakeLists.txt is newer than CMakeCache.txt
function Test-NeedsConfigure($buildDir) {
    $cacheFile = Join-Path $buildDir "CMakeCache.txt"
    if (-not (Test-Path $cacheFile)) { return $true }
    $cmakeLists = Join-Path $script:ProjectRoot "CMakeLists.txt"
    if (-not (Test-Path $cmakeLists)) { return $true }
    $cacheTime = (Get-Item $cacheFile).LastWriteTime
    $cmakeTime = (Get-Item $cmakeLists).LastWriteTime
    return ($cmakeTime -gt $cacheTime)
}
...
$script:Form.Text          = "Perfect Dark 2 - Dev Window v2"
...
# Bottom of build tab: GitHub + Folder + Clean Build + Prune buttons
...
$script:BtnCleanBuild.FlatAppearance.BorderColor = $script:ColorOrange
$script:BtnCleanBuild.FlatAppearance.BorderSize = 1
$script:BtnCleanBuild.ForeColor = $script:ColorOrange
$script:BtnCleanBuild.BackColor = [System.Drawing.Color]::FromArgb(58, 58, 58)
$script:BtnCleanBuild.Font = New-UIFont 12 -Bold
$script:BtnCleanBuild.Cursor = [System.Windows.Forms.Cursors]::Hand
$script:BtnCleanBuild.Add_Click({
    $script:ForceCleanBuild = $true
    Start-Build
})
$script:LinkPanel.Controls.Add($script:BtnCleanBuild)
...
# ============================================================================
# Section 15: Build pipeline (SMART BUILDS)
#
# SYNC RULE: The cmake flags in Get-BuildSteps MUST match build-headless.ps1.
...
function Start-PushRelease {
    ...
    # Release always does a clean build
...
# BUILD button: normal click = incremental, Shift+Click = force clean
...
function Invoke-FormResize {
    ...
    try {
        ...
        # Link panel (4 buttons across bottom)
        if ($null -ne $script:LinkPanel) {
            ...
            $numBtns = 4
            $lbw = [math]::Floor(($heroW - (($numBtns - 1) * 6)) / $numBtns)
            $lgap = 6
            $bh = $linkH - 8
            if ($null -ne $script:BtnOpenGitHub)      { $script:BtnOpenGitHub.Location      = New-Object System.Drawing.Point(0, 4); $script:BtnOpenGitHub.Size = New-Object System.Drawing.Size($lbw, $bh) }
            if ($null -ne $script:BtnOpenFolder)      { $script:BtnOpenFolder.Location      = New-Object System.Drawing.Point(($lbw + $lgap), 4); $script:BtnOpenFolder.Size = New-Object System.Drawing.Size($lbw, $bh) }
            if ($null -ne $script:BtnCleanBuild)      { $script:BtnCleanBuild.Location      = New-Object System.Drawing.Point(($lbw * 2 + $lgap * 2), 4); $script:BtnCleanBuild.Size = New-Object System.Drawing.Size($lbw, $bh) }
            if ($null -ne $script:BtnPruneWorktrees)  { $script:BtnPruneWorktrees.Location  = New-Object System.Drawing.Point(($lbw * 3 + $lgap * 3), 4); $script:BtnPruneWorktrees.Size = New-Object System.Drawing.Size($lbw, $bh) }
        }
