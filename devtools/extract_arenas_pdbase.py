...
def build_constant_table(constants_path, prefix_re):
    """Scan constants.h for #define <NAME> <VALUE> lines whose NAME matches
    prefix_re and resolve to integer values. Iterates to fixed point so
    forward refs (e.g. STAGE_BAR = 0x5e) and aliases resolve."""
    text = open(constants_path, "r", encoding="utf-8").read()
    pat = re.compile(
        r"^#define\s+(" + prefix_re.pattern + r"\w*)\s+([^\s/]+)",
        re.MULTILINE)
    raw = {}
    for m in pat.finditer(text):
        name, value = m.group(1), m.group(2).strip()
        # Strip trailing parentheses content from constants like "FOO(BAR)"
        # by skipping any line whose value contains a paren -- those are
        # macro invocations not pure constants.
        if "(" in value:
            continue
        raw[name] = value
    table = {}
...
def resolve_symbol(sym, *tables):
    """Resolve sym (str) to int by trying each provided table; returns
    sym unchanged if no match (caller decides how to handle)."""
...
# Lang enums L_MPMENU_NNN / L_OPTIONS_NNN are NOT in constants.h. They
# are auto-generated by tools/assetmgr/mklang into src/generated/<rom>/
# lang/{mpmenu,options}.h as `enum l_mpmenu { L_MPMENU_000 = 0x5000,
# L_MPMENU_001, ... };` -- sequential from the explicit base address.
#
# Bases per the headers:
#   L_MPMENU  starts at 0x5000
#   L_OPTIONS starts at 0x5600
#
# Both are dense sequential; index 119 = base + 119.
LANG_BASE_TABLE = {
    "L_MPMENU_": 0x5000,
    "L_OPTIONS_": 0x5600,
}

RE_LANG_NAME = re.compile(r"^(L_(?:MPMENU|OPTIONS)_)(\d+)$")


def resolve_lang_symbol(sym):
    """Compute the integer value of an L_MPMENU_NNN or L_OPTIONS_NNN
    symbol from its base + sequential offset. Returns sym unchanged if
    the name does not match the expected shape."""
    if isinstance(sym, int):
        return sym
    m = RE_LANG_NAME.match(sym)
    if not m:
        return sym
    prefix, idx_str = m.group(1), m.group(2)
    base = LANG_BASE_TABLE.get(prefix)
    if base is None:
        return sym
    try:
        idx = int(idx_str, 10)
...
def main():
    ...
    # Build STAGE_* and L_* tables. These are large families
    # (~hundreds of entries each) so we resolve at extract time and
    # store integer values in the JSON. ARENA_LOADMODE_* is small and
    # handled symbolically by the loader's inline resolver.
    stage_table = build_constant_table(args.constants, re.compile(r"STAGE_"))
    lang_table = {}
    lang_table.update(build_constant_table(args.constants, re.compile(r"L_OPTIONS_")))
    lang_table.update(build_constant_table(args.constants, re.compile(r"L_MPMENU_")))
    ...
    for entry in arenas:
        ...
        # Resolve STAGE_* and L_* symbolic names to integers (Path B
        # for these large enum families; ARENA_LOADMODE_* stays
        # symbolic since the inline resolver covers 3 entries).
        stagenum_resolved = resolve_symbol(entry["stagenum"], stage_table)
        if isinstance(stagenum_resolved, str):
            sys.stderr.write(
                "warning: unresolved STAGE_* symbol '%s' at arena_index %d\n"
                % (stagenum_resolved, idx))
            stagenum_resolved = 0
        # L_MPMENU_NNN / L_OPTIONS_NNN are auto-generated enum
        # sequences (mklang). Compute via the base+offset rule.
        name_resolved = resolve_lang_symbol(entry["name_langid"])
        if isinstance(name_resolved, str):
            sys.stderr.write(
                "warning: unresolved L_* symbol '%s' at arena_index %d\n"
                % (name_resolved, idx))
            name_resolved = 0
