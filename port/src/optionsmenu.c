...
static MenuItemHandlerResult menuhandlerPlayerJumpHeight(s32 operation, struct menuitem *item, union handlerdata *data)
{
	switch (operation) {
	case MENUOP_GETSLIDER:
		if (g_PlayerExtCfg[g_ExtMenuPlayer].jumpheight <= 0.f) {
			data->slider.value = 0; /* 0 = use match setting */
		} else if (g_PlayerExtCfg[g_ExtMenuPlayer].jumpheight > 20.f) {
			data->slider.value = 200;
		} else {
			data->slider.value = (s32)(g_PlayerExtCfg[g_ExtMenuPlayer].jumpheight * 10.f + 0.5f);
		}
		break;
	case MENUOP_SET:
		g_PlayerExtCfg[g_ExtMenuPlayer].jumpheight = (f32)data->slider.value / 10.f;
		break;
	case MENUOP_GETSLIDERLABEL:
		if (data->slider.value == 0) {
...
struct menuitem g_ExtendedGameMenuItems[] = {
	...
	},
	{
		...
		0,
		MENUITEMFLAG_LITERAL_TEXT | MENUITEMFLAG_SLIDER_WIDE,
		(uintptr_t)"Jump Height",
		200,
		menuhandlerPlayerJumpHeight,
	},
	{
		MENUITEMTYPE_SEPARATOR,
		0,
...
struct menudialogdef g_ExtPlayerGameOptionsMenuDialog = {
	MENUDIALOGTYPE_DEFAULT,
...
{
	if (operation == MENUOP_SET) {
		g_ExtNextDialog = &g_ExtPlayerGameOptionsMenuDialog;
		menuPushDialog(&g_ExtendedSelectPlayerMenuDialog);
	}
	return 0;
}
