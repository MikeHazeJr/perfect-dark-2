...
/* Configure TLS verification for an easy handle.
 *
 * Primary source of trust: the embedded Mozilla CA bundle (cacert_blob.h,
 * generated by CMake from port/src/cacert.pem). The statically-linked
 * libcurl.a in MSYS2 is built against OpenSSL WITHOUT winstore support, so
 * CURLSSLOPT_NATIVE_CA alone is insufficient on tester machines that lack
 * an OpenSSL-readable system bundle. Passing the bundle as an in-memory
 * blob keeps the binary zero-DLL and self-sufficient.
 *
 * CURLSSLOPT_NATIVE_CA is kept as a harmless secondary on builds that
