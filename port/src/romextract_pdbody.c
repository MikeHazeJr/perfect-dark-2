                          ...
                          const char *out_dir, s32 force_rewrite)
{
	...
	if (assetArchiveWriterInit(&asset_writer, aw, "body", catalog_id) !=
			MODARCHIVE_OK) {
		sysLoudFailf("EXTRACT.PDBODY",
	...
	assetArchiveWriterSetProvenance(&asset_writer, "romextract_pdbody",
		"bodydata_authored", (s32)b->bodynum, mesh_str ? mesh_str : "");

	if (assetArchiveWriterAddDescriptor(&asset_writer, "body.ini",
	...
	if (b->handfilenum != 0 &&
	    s_addRequiredArchiveFile(&asset_writer, "hand.pdmesh",
			hand_rel, full) != 0) {
		modArchiveAbort(aw);
		return -1;
	}
...
s32 romExtractAllPdbody(s32 force_rewrite)
{
	...
	s32 effective_force = force_rewrite |
		romExtractPdFastCacheKindMismatch(PDBODY_FAST_CACHE_KIND, bodies_dir);

	pdbody_fanout_ctx_t bctx;
	memset(&bctx, 0, sizeof(bctx));
	bctx.bodies_dir    = bodies_dir;
	bctx.force_rewrite = effective_force;
