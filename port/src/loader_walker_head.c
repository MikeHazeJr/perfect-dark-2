/* Specialized public head archive walker. Identity and authored values come
 * from head.ini; optional private metadata supplies generated slot provenance.
    ...
    const char *pd_kind, const char *id, const char *file_path)
{
    ...
    loaderWalkerEnvelopeInt(manifest, manifest_len, "headnum", &hint);
    if (hint < 0 || hint >= CATALOG_MGR_HEAD_COUNT) hint = -1;
    if (!assetCatalogRegisterBodyHeadArchive(file_path, id, 1, (s32)hint,
                            ...
                            loader_walker_kind_result_t *out)
{
    static const loader_walker_kind_desc_t desc = {
        "head", "heads", ".pdhead", /* always_invoke: */ 1, "head.ini", "head",
    };
