                                          ...
                                          char out_hex[SHA256_HEX_SIZE])
{
	...
	/* A nested archive reference is a filesystem/VFS source, not a disk
	 * filename. Use the same resolved bytes as descriptor/geometry loading;
	 * the byte helper preserves this API's canonical entry ordering/domain. */
	if (strstr(archive_path, "::")) {
		u32 archive_size = 0;
		void *archive_bytes = weaponGraphArchiveReadBytesFile(archive_path,
			&archive_size);
		s32 result = weaponGraphArchiveCanonicalSha256Bytes(archive_bytes,
			archive_size, out_hex);
		free(archive_bytes);
		return result;
	}
