...
{
	// In netplay, only the server handles autogun AI (targeting, rotation).
	// Client receives state via SVC_PROP_MOVE/SVC_PROP_SYNC checksums.
	if (g_NetMode == NETMODE_CLIENT) {
		return;
	}
	...
	// In netplay, only the server handles autogun targeting.
	// Clients skip target acquisition entirely — barrel rotation
	// will just hold at last known rotation from the position sync.
	if (g_NetMode == NETMODE_CLIENT) {
		return;
	}
...
{
	// In netplay, only the server handles autogun firing.
	// Damage is already blocked on clients (func0f0341dc guard), but this
	// also prevents phantom beam/spark/sound effects from desync'd targeting.
	if (g_NetMode == NETMODE_CLIENT) {
		return;
	}
...
{
	// In co-op, alarm activation is server-authoritative.
	// Clients receive SVC_ALARM from server.
	if (g_NetMode == NETMODE_CLIENT
	...
	if (g_AlarmTimer < 1) {
		...
		if (g_NetMode == NETMODE_SERVER
				&& (g_NetGameMode == NETGAMEMODE_COOP || g_NetGameMode == NETGAMEMODE_ANTI)) {
			netmsgSvcAlarmWrite(&g_NetMsgRel, 1);
		}
	}
}
...
void alarmDeactivate(void)
{
	...
	if (g_NetMode == NETMODE_SERVER
			&& (g_NetGameMode == NETGAMEMODE_COOP || g_NetGameMode == NETGAMEMODE_ANTI)) {
		netmsgSvcAlarmWrite(&g_NetMsgRel, 0);
	}
}
