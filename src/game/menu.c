...
void dialogTick(struct menudialog *dialog, struct menuinputs *inputs, u32 tickflags)
{
	...
	/*
	 * PC port: Update mouse focus BEFORE ticking items so that a click
	 * on an unfocused item first moves focus there, then the select
	 * fires on the correct (newly focused) item in the same frame.
	 */
	if ((tickflags & MENUTICKFLAG_DIALOGISCURRENT) && inputs->mousemoved) {
		dialogChangeItemFocusWithMouse(dialog, inputs->mousex, inputs->mousey);
	}
...
void menuProcessInput(void)
{
	...
	if (menu->playernum == 0) {
		...
		if (inputMouseIsEnabled() && !inputMouseIsLocked() && g_MenuMouseControl) {
			...
			/*
			 * PC port: A fresh mouse click should act as a select.
			 * The original code only set mouseheld (for slider dragging)
	...
	if (g_Menus[g_MpPlayerNum].curdialog) {
		...
		/* Verbose menu input logging for debugging */
		if (inputs.select || inputs.back || inputs.start) {
			struct menudialog *curdlg = g_Menus[g_MpPlayerNum].curdialog;
			s32 focustype = -1;
			u32 dlgflags = 0;
			if (curdlg) {
				dlgflags = curdlg->definition->flags;
				if (curdlg->focuseditem) {
					focustype = curdlg->focuseditem->type;
				}
			}
			sysLogPrintf(LOG_NOTE,
				"MENU_INPUT: sel=%d back=%d start=%d mouse=%d/%d focustype=%d dlgflags=0x%x depth=%d",
				inputs.select, inputs.back, inputs.start,
				inputs.mouseheld, inputs.mousemoved,
				focustype, dlgflags, g_Menus[g_MpPlayerNum].depth);
		}
