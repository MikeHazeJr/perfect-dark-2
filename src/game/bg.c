...
#ifdef AVOID_UB
											if (batch->type == VTXBATCHTYPE_XLU && texturenum >= 0 && texGetDefinition(texturenum)->surfacetype == SURFACETYPE_DEFAULT) {
#else
											if (batch->type == VTXBATCHTYPE_XLU && texGetDefinition(texturenum)->surfacetype == SURFACETYPE_DEFAULT) {
#endif
												hit = false;
											}
