...
s32 challengeForceUnlockSetupFeatures(struct mpsetup *setup, u8 *array, s32 len)
{
	...
	for (i = 0; i < mpGetNumStages(); i++) {
		if (modmgrGetArena(i)->stagenum == setup->stagenum) {
			s32 featurenum = modmgrGetArena(i)->requirefeature;
...
void challengeForceUnlockConfigFeatures(struct mpconfig *config, u8 *array, s32 len, s32 challengeindex)
{
	...
	for (i = 0; i < MAX_BOTS; i++) {
		...
		if (config->simulants[i].mpbodynum < modmgrGetTotalBodies()) {
			featurenum = modmgrGetBody(config->simulants[i].mpbodynum)->requirefeature;
		...
		if (config->simulants[i].mpheadnum < modmgrGetTotalHeads()) {
			featurenum = modmgrGetHead(config->simulants[i].mpheadnum)->requirefeature;
...
void challengeForceUnlockBotFeatures(void)
{
	...
	for (i = 0; i < ARRAYCOUNT(g_BotConfigsArray); i++) {
		...
		if (g_BotConfigsArray[i].base.mpbodynum < modmgrGetTotalBodies()) {
			s32 featurenum = modmgrGetBody(g_BotConfigsArray[i].base.mpbodynum)->requirefeature;
		...
		if (g_BotConfigsArray[i].base.mpheadnum < modmgrGetTotalHeads()) {
			s32 featurenum = modmgrGetHead(g_BotConfigsArray[i].base.mpheadnum)->requirefeature;
