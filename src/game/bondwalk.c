...
s32 bwalkTryMoveUpwards(f32 amount)
{
	...
	if (result == CDRESULT_NOCOLLISION) {
		...
		/* Log collision details when upward movement is blocked during a jump */
		struct prop *obstacle = cdGetObstacleProp();
		if (obstacle) {
			sysLogPrintf(LOG_NOTE,
				"JUMP_BLOCKED: bwalkTryMoveUpwards(%.2f) HIT prop type=%d "
				"pos=(%.1f,%.1f,%.1f) playerpos=(%.1f,%.1f,%.1f) "
				"radius=%.1f ymax=%.1f ymin=%.1f types=0x%x",
				amount,
				obstacle->type,
				obstacle->pos.x, obstacle->pos.y, obstacle->pos.z,
				g_Vars.currentplayer->prop->pos.x,
				g_Vars.currentplayer->prop->pos.y,
				g_Vars.currentplayer->prop->pos.z,
				radius, ymax, ymin, types);
			if (obstacle->type == PROPTYPE_OBJ && obstacle->obj) {
				sysLogPrintf(LOG_NOTE,
					"JUMP_BLOCKED: obstacle is OBJ model=%d flags=0x%x",
					obstacle->obj->modelnum, obstacle->obj->flags);
			}
		} else {
			sysLogPrintf(LOG_NOTE,
				"JUMP_BLOCKED: bwalkTryMoveUpwards(%.2f) HIT bg/world geometry "
				"playerpos=(%.1f,%.1f,%.1f) radius=%.1f ymax=%.1f ymin=%.1f",
				amount,
				g_Vars.currentplayer->prop->pos.x,
				g_Vars.currentplayer->prop->pos.y,
				g_Vars.currentplayer->prop->pos.z,
...
#if PC_CAPSULE_ENABLED
static void bwalkClampAirborneSideEntry(f32 *verticalDelta, f32 *newmanground,
		f32 *fallspeed)
{
	...
	if (!player || !player->prop || !verticalDelta || !newmanground || !fallspeed) {
		return;
	}

	if (*verticalDelta <= 0.1f) {
		return;
	}
	...
	if (safefrac < 1.0f) {
		...
		RoomNum rooms[8];
		s32 geomtype = sweep.hittype == CAPSULE_HIT_PROP
		...
		if (geomtype == CAPSULE_HIT_CEILING) {
			*verticalDelta *= clampfrac;
			*newmanground = player->vv_manground + *verticalDelta;
			*fallspeed = 0.0f;
		}

		if (g_JumpLoggingEnabled) {
			sysLogPrintf(LOG_NOTE,
				"JUMP_SIDE_SWEEP: hit frac=%.3f clamp=%.3f type=%d rendered=%d "
				"lateral=(%.2f,%.2f) vertical=%.2f",
				safefrac, clampfrac, geomtype, sweep.hitfromrendered,
				lateralMove.x, lateralMove.z, *verticalDelta);
		}
	}
}
...
void bwalkUpdateVertical(void)
{
	...
	if (g_Vars.currentplayer->wantsjump) {
		...
		sysLogPrintf(LOG_NOTE, "JUMP: impulse=%.1f grounded=%d gap=%.2f "
			"manground=%.2f ground=%.2f bdeltaY=%.2f onladder=%d "
			"isfalling=%d inlift=%d",
			impulse, grounded, groundgap,
			g_Vars.currentplayer->vv_manground, g_Vars.currentplayer->vv_ground,
			g_Vars.currentplayer->bdeltapos.y, g_Vars.currentplayer->onladder,
			g_Vars.currentplayer->isfalling, g_Vars.currentplayer->inlift);

		if (impulse > 0.0f && grounded) {
			/* Clear lift state so free-fall physics take over. */
			 ...
			 * fires on the very next tick instead of being smoothed
			 * back to ground. */
			if (g_Vars.currentplayer->vv_manground <= g_Vars.currentplayer->vv_ground + 1.0f) {
				g_Vars.currentplayer->vv_manground = g_Vars.currentplayer->vv_ground + 3.0f;
			}

			/* Also update prop pos.y immediately so collision tests
			 * in the same frame use the post-jump position. */
			g_Vars.currentplayer->prop->pos.y =
				g_Vars.currentplayer->vv_manground + g_Vars.currentplayer->vv_height;

			sysLogPrintf(LOG_NOTE, "JUMP: APPLIED impulse=%.1f newManground=%.2f newPosY=%.2f",
				impulse, g_Vars.currentplayer->vv_manground,
				g_Vars.currentplayer->prop->pos.y);
		} else {
			sysLogPrintf(LOG_NOTE, "JUMP: BLOCKED impulse=%.1f grounded=%d gap=%.2f vel=%.2f",
				impulse, grounded, groundgap, g_Vars.currentplayer->bdeltapos.y);
		}
	} else {
		g_Vars.currentplayer->wantsjump = false;
	}
	...
	if (g_Vars.currentplayer->vv_manground > g_Vars.currentplayer->vv_ground) {
		...
		{
		/* PC: Swept capsule cast — project the player capsule along the
		 * vertical movement path BEFORE translating. If the sweep finds a
		 * collision before the full distance, clamp the movement to the
		 * safe fraction. This catches ceilings, wall planes above the
...
#if PC_CAPSULE_ENABLED
		bwalkClampAirborneSideEntry(&verticalDelta, &newmanground, &fallspeed);

		{
			...
			if (safefrac < 1.0f) {
				/* Sweep found a collision — clamp movement */
				f32 clampedDelta = verticalDelta * safefrac;
				sysLogPrintf(LOG_NOTE,
					"CAPSULE_SWEEP: hit %s at frac=%.3f, clamping move %.2f -> %.2f "
					"hittype=%d geoflags=0x%x prop=%s",
					sweep.hittype == CAPSULE_HIT_CEILING ? "CEILING" :
					sweep.hittype == CAPSULE_HIT_FLOOR ? "FLOOR" :
					sweep.hittype == CAPSULE_HIT_WALL ? "WALL" : "PROP",
					safefrac, verticalDelta, clampedDelta,
					sweep.hittype, sweep.hitgeoflags,
					sweep.hitprop ? "yes" : "no");
				newmanground = g_Vars.currentplayer->vv_manground + clampedDelta;
		...
		if (moveresult == CDRESULT_NOCOLLISION) {
			 ...
			 * The capsule ceiling probe tests the full collision volume
			 * upward, catching all geometry types. */
			if (fallspeed > 0.0f) {
				struct coord ceilpos;
				struct prop *ceilprop = NULL;
				...
				f32 ceilY = capsuleFindCeilingForProp(g_Vars.currentplayer->prop, &ceilpos, radius,
						...
						ymax - g_Vars.currentplayer->prop->pos.y,
						g_Vars.currentplayer->prop->rooms,
						g_Vars.bondcollisions ? CDTYPE_ALL : CDTYPE_BG,
						&ceilprop);

				f32 headroom = ceilY - g_Vars.currentplayer->vv_manground;
				f32 headheight = g_Vars.currentplayer->vv_headheight
						+ g_Vars.currentplayer->crouchoffsetrealsmall;
				if (headheight < 80.0f) {
					headheight = 80.0f;
				...
				if (ceilY < 99990.0f && headroom < headheight) {
					...
					if (clampedground < g_Vars.currentplayer->vv_ground) {
						clampedground = g_Vars.currentplayer->vv_ground;
					}
					sysLogPrintf(LOG_NOTE,
						"CAPSULE_CEIL_CLAMP: clamping manground %.1f -> %.1f "
						"(ceilY=%.1f headheight=%.1f) KILLING velocity",
						g_Vars.currentplayer->vv_manground, clampedground,
						ceilY, headheight);
					g_Vars.currentplayer->vv_manground = clampedground;
					g_Vars.currentplayer->prop->pos.y = clampedground;
					g_Vars.currentplayer->bdeltapos.y = 0.0f;
				}
			}
#endif /* PC_CAPSULE_ENABLED */
			...
			// Not falling - upward move was BLOCKED by collision
			sysLogPrintf(LOG_NOTE,
				"JUMP_COLLIDED: upward move blocked! manground=%.1f ground=%.1f "
				"vel=%.2f -> setting vel=0 isfalling=%d",
		...
		} /* close bwalkTryMoveUpwards scope */
	} else {
		// Not falling
		if (g_Vars.currentplayer->isfalling) {
			g_Vars.currentplayer->isfalling = false;
...
void bwalkApplyAdsSpeed(void)
{
	// Modern ADS: slow the player by 50% while aiming so they can still move
	// but at a controlled pace, consistent with contemporary shooter feel.
