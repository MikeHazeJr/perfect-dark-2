...
MenuItemHandlerResult menuhandlerMpEndGame(s32 operation, struct menuitem *item, union handlerdata *data)
{
	if (operation == MENUOP_SET) {
		...
		// Server: mainEndStage() broadcasts SVC_STAGE_END via netServerStageEnd()
		mainEndStage();
	}
...
static MenuItemHandlerResult menuhandlerNetTeamSwitch(s32 operation, struct menuitem *item, union handlerdata *data)
{
	...
	case MENUOP_GETOPTIONCOUNT:
		...
		break;
	case MENUOP_GETOPTIONTEXT:
		if (data->dropdown.value >= MAX_TEAMS) {
			data->dropdown.value = 0;
		}
		return (uintptr_t) &g_BossFile.teamnames[data->dropdown.value];
	case MENUOP_SET:
		if (g_NetLocalClient && data->dropdown.value < MAX_TEAMS) {
			g_NetLocalClient->settings.team = data->dropdown.value;
			// update local player config
			if (g_NetLocalClient->config) {
				g_NetLocalClient->config->base.team = data->dropdown.value;
			}
	...
	case MENUOP_GETSELECTEDINDEX:
		if (g_NetLocalClient) {
			data->dropdown.value = g_NetLocalClient->settings.team;
			if (data->dropdown.value >= MAX_TEAMS) {
				data->dropdown.value = 0;
			}
		}
...
static MenuItemHandlerResult menuhandlerNetPauseControls(s32 operation, struct menuitem *item, union handlerdata *data)
{
	...
	if (operation == MENUOP_SET) {
		menuPushDialog(&g_NetPauseControlsMenuDialog);
	}
	return 0;
}
...
struct menudialogdef g_MpPauseControlMenuDialog = {
	...
	NULL,
	MENUDIALOGFLAG_SMOOTHSCROLLABLE, // Enable scrolling — menu has many items
	NULL,
};
