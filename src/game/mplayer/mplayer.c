 ...
 * Play / Mission Select) and in mainEndStage's forge-active branch
 ...
 *    4 = reserved/deprecated (clamped to top-level by the UI)
 *
...
struct mpweapon g_MpWeapons[NUM_MPWEAPONS] = {
	...
	// 8 Goldfinger 64 imports (PP9I..RCP45, old slots 0x27..0x2e)
	// removed 2026-04-26. SHIELD/DISABLED shifted down from 0x2f/0x30
	// to 0x27/0x28.
	/*0x27*/ { WEAPON_MPSHIELD,         0,                    0,   0,                   0,  1, MPFEATURE_WEAPON_SHIELD,          MODEL_CHRSHIELD,        256 },
	/*0x28*/ { WEAPON_DISABLED }, // 0x25 on N64
};
...
void mpStartMatch(void)
{
	...
	/* Stage material rules are applied from the selected public level graph
	 * at the level transition boundary, after texture caches reset. */
...
Gfx *mpRenderModalText(Gfx *gdl)
{
	...
	if (g_MpSetup.paused == MPPAUSEMODE_PAUSED) {
		...
		gdl = text0f153780(gdl);
			...
			&& !(g_Vars.coopplayernum >= 0 && ((g_Vars.bond->isdead && g_Vars.coop->isdead) || !g_Vars.currentplayer->coopcanrestart || playerAnyInCutscene()))
			&& !(g_Vars.antiplayernum >= 0 && ((g_Vars.currentplayer != g_Vars.anti || playerAnyInCutscene())))
