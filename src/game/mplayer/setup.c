...
MenuItemHandlerResult mpCharacterBodyMenuHandler(s32 operation, struct menuitem *item, union handlerdata *data, s32 mpbodynum, s32 mpheadnum, bool isplayer)
{
...
#if VERSION >= VERSION_NTSC_1_0
	case MENUOP_FOCUS:
		if (g_Menus[g_MpPlayerNum].menumodel.allocstart == NULL) {
			// returning after gunmem was freed — textures need full reload
	...
	case MENUOP_CHECKPREFOCUSED:
		...
		/* PC: Push model to far right of dialog so it doesn't
		 * overlap the narrowed body list on the left. */
		g_Menus[g_MpPlayerNum].menumodel.curposx = 35.0f;
		g_Menus[g_MpPlayerNum].menumodel.newposx = 35.0f;

		g_Menus[g_MpPlayerNum].menumodel.curposy = -4.5f;
		g_Menus[g_MpPlayerNum].menumodel.newposy = -4.5f;

		g_Menus[g_MpPlayerNum].menumodel.curscale = 0.0018f;
...
// Preview state for character select — updated on scroll, committed on A press
static s32 s_PreviewBodyNum = -1;
static s32 s_PreviewHeadNum = -1;

/**
 * Check if the given mpbodynum refers to a body with an integrated head
 * (unk00_01 == 1), meaning no separate head model can be swapped in.
 * Examples: Dr. Carroll (floating robot), Eye Spy, Skedar, etc.
 */
static bool mpBodyHasIntegratedHead(s32 mpbodynum)
{
	if (mpbodynum < 0 || mpbodynum >= modmgrGetTotalBodies()) {
		return false;
	}
...
{
	s32 mpbodynum = g_PlayerConfigsArray[g_MpPlayerNum].base.mpbodynum;

	// Bodies with integrated heads (unk00_01=1) don't support head swapping.
	// Lock the carousel to prevent crashes from invalid head+body combos.
	if (mpBodyHasIntegratedHead(mpbodynum)) {
		switch (operation) {
		case MENUOP_GETOPTIONCOUNT:
			data->carousel.value = 1; // Show exactly 1 "head" option (the assigned one)
			return 0;
		case MENUOP_SET:
			return 0; // Don't allow head changes
		case MENUOP_GETSELECTEDINDEX:
			data->carousel.value = 0; // Always index 0
			return 0;
		}
	}
...
//
// Layout: Two scrollable lists stacked vertically on the left (body + head),
// with a 3D model preview on the right. Preview updates on scroll but only
// commits (locks in) the selection when the player presses A.
//
// Preview state is tracked separately from committed state so the model
// updates live as the user browses.
// --------------------------------------------------------------------------

MenuItemHandlerResult mpCharacterBodyListHandler(s32 operation, struct menuitem *item, union handlerdata *data)
{
	switch (operation) {
	case MENUOP_GETOPTIONCOUNT:
		data->list.value = mpGetNumBodies();
		break;
	case MENUOP_GETOPTIONTEXT:
		return (uintptr_t)mpGetBodyName(data->list.value);
	case MENUOP_LISTITEMFOCUS:
	...
	case MENUOP_SET:
		...
		break;
	case MENUOP_GETSELECTEDINDEX:
		data->list.value = g_PlayerConfigsArray[g_MpPlayerNum].base.mpbodynum;
		break;
	case MENUOP_21:
		if (!challengeIsFeatureUnlocked(mpGetBodyRequiredFeature(data->list.value))) {
			return 1;
		}
		break;
	}
...
// Head carousel handler — wraps mpCharacterHeadMenuHandler with preview state.
// Uses a carousel (not a list) because heads have no text names in the game
// data — they're displayed as 3D models. The carousel fires MENUOP_SET on
// each scroll, which naturally updates the preview.
// For bodies with integrated heads (unk00_01=1), the carousel is locked.

// Dialog handler for character select — drives the 3D model preview
MenuDialogHandlerResult mpCharacterSelectDialogHandler(s32 operation, struct menudialogdef *dialogdef, union handlerdata *data)
{
	switch (operation) {
	case MENUOP_OPEN:
		// Initialize preview state from committed config
		s_PreviewBodyNum = g_PlayerConfigsArray[g_MpPlayerNum].base.mpbodynum;
		s_PreviewHeadNum = g_PlayerConfigsArray[g_MpPlayerNum].base.mpheadnum;
		{
			union handlerdata initdata;
			mpCharacterBodyMenuHandler(MENUOP_SET, &dialogdef->items[0], &initdata,
					s_PreviewBodyNum, s_PreviewHeadNum, true);
		}
		break;
	case MENUOP_TICK:
		// Continuously update 3D model preview using preview state
		{
			s32 bodynum = s_PreviewBodyNum >= 0 ? s_PreviewBodyNum : g_PlayerConfigsArray[g_MpPlayerNum].base.mpbodynum;
			s32 headnum = s_PreviewHeadNum >= 0 ? s_PreviewHeadNum : g_PlayerConfigsArray[g_MpPlayerNum].base.mpheadnum;
			union handlerdata tickdata;
			mpCharacterBodyMenuHandler(MENUOP_11, &dialogdef->items[0], &tickdata,
					bodynum, headnum, true);
		}
		break;
	}
...
struct menuitem g_MpCharacterMenuItems[] = {
	/* Body list — fills the left side of a wide dialog.
	 * param2 = width (0x68 = 104px), param3 = height (0x90 = 144px).
	 * The 3D model preview is pushed to the far right via curposx
	 * so it doesn't overlap the text.  Tall enough for ~8 entries. */
	{
		MENUITEMTYPE_LIST,
		0,
		MENUITEMFLAG_LIST_WIDE,
		0x00000068,
		0x00000090,
		mpCharacterBodyListHandler,
	},
	/* Head carousel — sits below the body list.
	 * For integrated-head bodies the handler locks to 1 option. */
	{
...
struct menudialogdef g_MpCharacterMenuDialog = {
	...
	mpCharacterSelectDialogHandler,
	MENUDIALOGFLAG_0002,
	NULL,
};
