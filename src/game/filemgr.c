...
void filemgrPushSelectLocationDialog(s32 arg0, u32 filetype)
{
	...
	/*
	 * PC port: Only one save device exists (Game Pak / EEPROM emulation).
	 * Controller Paks are stubbed out and report PFS_ERR_NOPACK, making
	 * their menu items permanently disabled and the dialog unusable.
	 * Skip the location dialog entirely and auto-select SAVEDEVICE_GAMEPAK.
	 *
	 * We call the save functions directly rather than filemgrSaveToDevice()
	 * because that function checks spacesfree > 0, which is 0 on a clean
	 * EEPROM filesystem (spacesfree only counts reusable corrupted slots,
	 * not whether new files can be allocated).
	 */
	g_Menus[g_MpPlayerNum].fm.device2 = SAVEDEVICE_GAMEPAK;

	if (arg0 == 0) {
		filemgrSaveGameToDevice(SAVEDEVICE_GAMEPAK);
	} else if (arg0 == 6) {
		struct fileguid guid;
		guid.fileid = g_FileLists[g_Menus[g_MpPlayerNum].fm.listnum]->deviceguids[SAVEDEVICE_GAMEPAK].fileid;
		guid.deviceserial = g_FileLists[g_Menus[g_MpPlayerNum].fm.listnum]->deviceguids[SAVEDEVICE_GAMEPAK].deviceserial;
		filemgrSaveOrLoad(&guid, FILEOP_SAVE_MPPLAYER, (u32)g_MpPlayerNum);
	} else if (arg0 == 7) {
		struct fileguid guid;
		guid.fileid = g_FileLists[g_Menus[g_MpPlayerNum].fm.listnum]->deviceguids[SAVEDEVICE_GAMEPAK].fileid;
