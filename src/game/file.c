...
void *fileLoadRomToNew(s32 filenum, u32 method, u32 loadtype)
{
	...
	if (method == FILELOADMETHOD_EXTRAMEM || method == FILELOADMETHOD_DEFAULT) {
		...
		/* B-164 torn-models fix (2026-04-18): always recompute. fileLoad()
		 * overwrites info->loadedsize with the post-preprocess size on exit,
		 * stripping the +0x8000 rzipInflate scratch buffer. A cached value
		 * on a subsequent load allocates only the post-preprocess size; the
		 * scratch region then lands inside dst, in-place inflate corrupts
		 * the deflate stream as the writer catches the reader, rzipInflate
		 * yields 0, and dst stays zeroed → modeldef numparts=0/rootnode=NULL
		 * ("loaded torn") on every re-entry. */
