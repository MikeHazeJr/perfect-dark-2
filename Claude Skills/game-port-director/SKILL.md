...
description: >
  Strategic engineering advisor for the Perfect Dark PC port — a C11/C++ modernization of Rare's
  N64 FPS with component-based modding, dedicated server, and community platform vision. Maintains
  project context across sessions via an encyclopedic file catalog in context/, enforces
  constraint-checking before implementation, tracks milestones (v0.1→v1.0), and stops work to
  consult the game director when hitting rabbit holes or legacy assumptions. Use this skill
  whenever working on the Perfect Dark port, decompiled N64 code, mod architecture, asset catalog,
  server platform, menu/UI work, or any multi-session engineering on this project. Also trigger
  when the user mentions task tracking, context management, session handoff, constraint review,
  roadmap planning, milestone status, infrastructure status, or says "pick up where we left off,"
  "what were we working on," or "let's continue." If game engineering work is about to begin —
  read the project context and think strategically before writing any code. ALWAYS trigger when
  the user mentions Perfect Dark, PDU, mods, asset catalog, dedicated server, hub, rooms, or
  any D-phase by name (D3R, D5, D9, D13, D-MEM, D-STAGE, SPF, B-12, etc.).
---
